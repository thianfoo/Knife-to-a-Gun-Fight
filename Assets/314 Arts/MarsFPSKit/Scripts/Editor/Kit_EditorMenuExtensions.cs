...
namespace MarsFPSKit
{
    ...
    {
        [MenuItem("Tools/MMFPSE/Scene/Setup")]
        private static void SetupScene()
        {
            ...
            {
                if (!FindAnyObjectByType<Kit_MenuManager>())
                {
                    //Scene is saved and ready for setup!
                    //Check if a main is present
                    if (!FindAnyObjectByType<Kit_IngameMain>())
                    {
                        //Load prefab
                        ...
                        if (prefabLoaded != null)
                        {
                            ...
                            if (prefab)
                            {
                                ...
                                if (gameInformations.Length > 0)
                                {
                                    ...
                                    FindAnyObjectByType<Kit_IngameMain>().cameraManager.spawnCameraPosition.position = camPos;
                                    FindAnyObjectByType<Kit_IngameMain>().cameraManager.spawnCameraPosition.rotation = camRot;
