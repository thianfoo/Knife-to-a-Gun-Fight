...
namespace MarsFPSKit
{
    ...
    /// </summary>
    public class Kit_OutOfMapTrigger : Kit_Base
    {
        [Range(1f, 60f)]
        /// <summary>
        ...
        private void OnTriggerEnter(Collider other)
        {
            ...
            if (pb)
            {
                ...
                if (pb.isLocalPlayer && !pb.isBot)
                {
                    ...
                    main.hud.DisplayLeavingBattlefield(timeUntilDeath);
                }
            }
        }
                ...
                for (int i = 0; i < playersOutOfMap.Count; i++)
                {
                    if (playersOutOfMap[i].pb == pb)
                    {
                        if (playersOutOfMap[i].pb.isLocalPlayer && !playersOutOfMap[i].pb.isBot)
                        {
                            ...
                            main.hud.DisplayLeavingBattlefield(-1);
                        }
        ...
        private void Update()
        {
            ...
            for (int i = playersOutOfMap.Count - 1; i >= 0; i--)
            {
                if (playersOutOfMap[i].pb)
                {
                    ...
                    {
                        //Update hud
                        main.hud.DisplayLeavingBattlefield(playersOutOfMap[i].timeLeft);
                    }
                }
                else
                {
