    ...
    public class Kit_PlayerHUD : Kit_PlayerHUDBase
    {
        ...
        public override void DisplayWeaponPickup(bool displayed, int weapon = -1)
        {
            if (displayed)
            {
                ...
                {
                    //Set name
                    weaponPickupText.text = string.Format(weaponPickupLocalization.GetLocalizedString(), main.gameInformation.allWeapons[weapon].weaponName.GetLocalizedString());
                }
            }
            else
            {
        ...
        {
            //Get screen pos
            Vector3 canvasPos = canvas.WorldToCanvas(worldPos, main.cameraManager.mainCamera);
            //Set
