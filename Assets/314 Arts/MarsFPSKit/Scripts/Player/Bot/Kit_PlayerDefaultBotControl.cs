...
namespace MarsFPSKit
{
    ...
    public class Kit_PlayerDefaultBotControl : Kit_PlayerBotControlBase
    {
        ...
        {
            Kit_PlayerDefaultBotControlData kbcrd = new GameObject().AddComponent<Kit_PlayerDefaultBotControlData>();
        ...
        {
            Kit_PlayerDefaultBotControlData kbcrd = pb.botControlsRuntimeData as Kit_PlayerDefaultBotControlData;
            ...
            if (pb.enableInput)
            {
                ...
                if (Time.time - 0.1f > kbcrd.lastRoutine)
                {
                    ...
                    if (Time.time - 5f > kbcrd.lastForceNewWaypoint || !kbcrd.playerToEngage && (GetRemainingDistance(kbcrd) < 1f || !kbcrd.nma.hasPath) || kbcrd.playerToEngage && (GetRemainingDistance(kbcrd) < 1f))
                    {
        ...
        private void SetDestination(Kit_PlayerDefaultBotControlData kbcrd, Vector3 position)
        {
        ...
        private float GetRemainingDistance(Kit_PlayerDefaultBotControlData kbcrd)
        {
            if (kbcrd.nma.isOnNavMesh)
            {
        ...
        void ReachedDestination(Kit_PlayerBehaviour pb, Kit_PlayerDefaultBotControlData kbcrd)
        {
            if (!kbcrd.playerToEngage)
            {
                //Set new waypoint
            ...
            else
            {
                ...
                kbcrd.reachedDestination = false;
                /*
                SetDestination(kbcrd, kbcrd.playerToEngage.transform.position + (Random.insideUnitSphere * Random.Range(0, 3f)));
                kbcrd.reachedDestination = false;
                */
            }
            kbcrd.lastForceNewWaypoint = Time.time;
        ...
        void CheckForEnemies(Kit_PlayerBehaviour pb, Kit_PlayerDefaultBotControlData kbcrd)
        {
        ...
        bool CanSeePlayer(Kit_PlayerBehaviour pb, Kit_PlayerDefaultBotControlData kbcrd, GameObject enemyPlayer)
        {
            if (enemyPlayer)
            {
        ...
        bool isEnemyPlayer(Kit_PlayerBehaviour pb, Kit_PlayerDefaultBotControlData kbcrd, GameObject enemyPlayer)
        {
            if (pb)
            {
        ...
        void UpdateAimPosition(Kit_PlayerBehaviour pb, Kit_PlayerDefaultBotControlData kbcrd)
        {
            if (pb.input.sprint)
            {
        ...
        void FiringDecision(Kit_PlayerBehaviour pb, Kit_PlayerDefaultBotControlData kbcrd)
        {
        ...
        void UpdatePlayerToEngage(Kit_PlayerBehaviour pb, Kit_PlayerDefaultBotControlData kbcrd)
        {
        ...
        void FullAutoFire(Kit_PlayerBehaviour pb, Kit_PlayerDefaultBotControlData kbcrd)
        {
            pb.input.lmb = true;
        }

        void BurstFire(Kit_PlayerBehaviour pb, Kit_PlayerDefaultBotControlData kbcrd)
        {
        ...
        void SingleFire(Kit_PlayerBehaviour pb, Kit_PlayerDefaultBotControlData kbcrd)
        {
            if (Time.time > kbcrd.lastShot)
            {
                pb.input.lmb = false;
        ...
        bool isAimingNearEngage(Kit_PlayerBehaviour pb, Kit_PlayerDefaultBotControlData kbcrd)
        {
            if (kbcrd.playerToEngage)
            {
                Kit_PlayerBehaviour enemyPlayer = kbcrd.playerToEngage.GetComponent<Kit_PlayerBehaviour>();
