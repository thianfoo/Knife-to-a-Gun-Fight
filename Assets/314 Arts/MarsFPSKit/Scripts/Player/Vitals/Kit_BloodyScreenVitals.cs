...
namespace MarsFPSKit
{
    ...
    public class Kit_BloodyScreenVitals : Kit_VitalsBase
    {
                ...
                if (!pb.spawnProtection || pb.spawnProtection.CanTakeDamage(pb))
                {
                    ...
                    if (vrd.hitPoints > 0)
                    {
                        ...
                        if (hitReactionEnabled)
                        {
                            ...
                            coroutineHelper.StartCoroutine(coroutineHelper.Kick(pb.playerCameraHitReactionsTransform, dir, 0.1f));

                            coroutineHelper.StartCoroutine(coroutineHelper.Kick(pb.weaponsHitReactions, dir * 2f, 0.1f));
                        }
                        //Set damage effect
                        vrd.hitAlpha = 2f;
                        //Play voice
        ...
        public override void CustomUpdate(Kit_PlayerBehaviour pb)
        {
            ...
            if (vrd)
            {
                ...
                if (pb.isFirstPersonActive)
                {
                    ...
                    {
                        //Update hud
                        main.hud.DisplayHealth(vrd.hitPoints);
                        main.hud.DisplayHurtState(vrd.hitAlpha);
                    }
                    else
                    {
                        //Update hud with negative values, so its hidden
                        main.hud.DisplayHealth(-1);
                        //Negative values should hide it
                        main.hud.DisplayHurtState(1 - vrd.hitPoints / 100f);
                    }
                }
                ...
                if (pb.isServer)
                {
                    ...
                    if (pb.transform.position.y <= main.mapDeathThreshold)
                    {
        ...
        public override void InitializeClient(Kit_PlayerBehaviour pb)
        {
            ...
            {
                //Find ours
                pb.vitalsNetworkData = FindObjectsByType<Kit_BloodyScreenVitalsNetworkData>(FindObjectsSortMode.None).Where(x => x.ownerPlayerNetworkId == pb.netId).FirstOrDefault();
            }
        }
    }
}
