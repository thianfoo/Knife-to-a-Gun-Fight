    ...
    namespace Weapons
    {
        ...
        public class Kit_ModernWeaponManager : Kit_WeaponManagerBase
        {
            ...
            public override void OnWeaponsInUseSyncCallback(Kit_PlayerBehaviour pb, SyncList<uint>.Operation op, int itemIndex, uint item)
            {
                ...
                switch (op)
                {
                    case SyncList<uint>.Operation.OP_SET:
                        ...
                        coroutineHelper.StartCoroutine(coroutineHelper.NetworkReplaceWeapon(pb, runtimeData, itemIndex, runtimeData.weaponsInUseSync[itemIndex]));
                        break;
                }
            }
            ...
            public override void SetupSpawnData(Kit_PlayerBehaviour pb, Loadout loadout, NetworkConnectionToClient sender)
            {
                ...
                {
                    var loadoutWeapon = loadout.loadoutWeapons[i];

                    if (main.gameInformation.allWeapons.Length > loadoutWeapon.weaponID)
                    {
                        GameObject spawnObject = main.gameInformation.allWeapons[loadoutWeapon.weaponID].SetupSpawnData(pb, i, loadoutWeapon);
                    ...
                    {
                        DebugLogError("Trying to spawn with out of bounds weapon: " + loadoutWeapon.weaponID);
                    }
                }
            }
            ...
            public override void SetupSpawnData(Kit_PlayerBehaviour pb, Loadout loadout)
            {
                ...
                for (int i = 0; i < loadout.loadoutWeapons.Length; i++)
                {
                    GameObject spawnObject = main.gameInformation.allWeapons[loadout.loadoutWeapons[i].weaponID].SetupSpawnData(pb, i, loadout.loadoutWeapons[i]);
            ...
            public override void InitializeServer(Kit_PlayerBehaviour pb)
            {
                ...
                for (int i = 0; i < runtimeData.weaponsInUseDataObjects.Length; i++)
                {
                    ...
                    runtimeData.weaponsInUseDataObjects[slot].pb = pb;
                    //Setup FP
            ...
            public override void InitializeClient(Kit_PlayerBehaviour pb)
            {
                ...
                {
                    //Find ours
                    pb.weaponManagerNetworkData = FindObjectsByType<Kit_ModernWeaponManagerNetworkData>(FindObjectsSortMode.None).Where(x => x.ownerPlayerNetworkId == pb.netId).FirstOrDefault();
                }
            ...
            {
                //Hide crosshair
                main.hud.DisplayCrosshair(0f, false);

            }

            public override void ForceUnselectCurrentWeapon(Kit_PlayerBehaviour pb)
            {
                DebugLog("[Weapon Manager] Forcing unselect of current weapon!");
                Kit_ModernWeaponManagerNetworkData runtimeData = pb.weaponManagerNetworkData as Kit_ModernWeaponManagerNetworkData;
                ...
                {
                    DebugLogError("Could not find next weapon! This is not allowed!");
                }
            }
            ...
            public override void PredictionInput(Kit_PlayerBehaviour pb, Kit_PlayerInput input, float delta)
            {
                ...
                if (Physics.Raycast(pb.playerCameraTransform.position, pb.playerCameraTransform.forward, out runtimeData.hit, pickupDistance, pickupLayers.value))
                {
                    if (runtimeData.hit.transform.root.GetComponent<Kit_DropBehaviour>())
                    {
                        ...
                        if (pb.isFirstPersonActive)
                        {
                            main.hud.DisplayWeaponPickup(true, drop.weaponID);
                            main.hud.DisplayInteraction(false);
                        }
                    ...
                    else if (runtimeData.hit.transform.GetComponentInParent<Kit_InteractableObject>())
                    {
                        ...
                        if (io.CanInteract(pb))
                        {
                            if (pb.isFirstPersonActive)
                            {
                                main.hud.DisplayWeaponPickup(false);
                                main.hud.DisplayInteraction(true, io.interactionText);
                            }
            ...
            public override void CustomUpdate(Kit_PlayerBehaviour pb, double revertBy)
            {
                ...
                if (pb.isFirstPersonActive)
                {
                    main.hud.DisplayWeaponsAndQuickUses(pb, runtimeData);
                }
            }

            public override void CustomLateUpdate(Kit_PlayerBehaviour pb, double revertTime)
            {
                Kit_ModernWeaponManagerNetworkData runtimeData = pb.weaponManagerNetworkData as Kit_ModernWeaponManagerNetworkData;

                //Update weapon animation
                runtimeData.weaponsInUseDataObjects[runtimeData.currentWeapon].behaviour.CalculateWeaponLateUpdate(pb, runtimeData.weaponsInUseDataObjects[runtimeData.currentWeapon]);
            }

            public override void CustomFixedUpdate(Kit_PlayerBehaviour pb, double revertTime)
            {
                Kit_ModernWeaponManagerNetworkData runtimeData = pb.weaponManagerNetworkData as Kit_ModernWeaponManagerNetworkData;

                //Update weapon animation
                runtimeData.weaponsInUseDataObjects[runtimeData.currentWeapon].behaviour.CalculateWeaponFixedUpdate(pb, runtimeData.weaponsInUseDataObjects[runtimeData.currentWeapon]);
            }

            public override void PlayerDead(Kit_PlayerBehaviour pb)
            {
                if (NetworkServer.active && (main.currentPvPGameModeBehaviour && main.currentPvPGameModeBehaviour.CanDropWeapons()) || (main.currentPvEGameModeBehaviour && main.currentPvEGameModeBehaviour.CanDropWeapons()))
                {
                    Kit_ModernWeaponManagerNetworkData runtimeData = pb.weaponManagerNetworkData as Kit_ModernWeaponManagerNetworkData;
            ...
            public override void FallDownEffect(Kit_PlayerBehaviour pb, bool wasFallDamageApplied)
            {
                ...
                if (!runtimeData.weaponsInUseDataObjects[runtimeData.currentWeapon]) return;
                if (!runtimeData.weaponsInUseDataObjects[runtimeData.currentWeapon].behaviour) return;
            ...
            public override void NetworkReplaceWeapon(Kit_PlayerBehaviour pb, int slot, int weapon, int bulletsLeft, int bulletsLeftToReload, int[] attachments)
            {
                ...
                GameObject spawnObject = main.gameInformation.allWeapons[weapon].SetupSpawnData(pb, slot, new LoadoutWeapon { weaponID = weapon, attachments = attachments });
                ...
                NetworkServer.Spawn(spawnObject, pb.gameObject);
                NetworkIdentity id = spawnObject.GetComponent<NetworkIdentity>();
            ...
            {
                Kit_ModernWeaponManagerNetworkData runtimeData = pb.weaponManagerNetworkData as Kit_ModernWeaponManagerNetworkData;
                if (runtimeData.weaponsInUseDataObjects.Length > runtimeData.currentWeapon)
                    //Relay to weapon script
            ...
            public override void DropWeapon(Kit_PlayerBehaviour pb, int slot)
            {
                if ((main.currentPvPGameModeBehaviour && main.currentPvPGameModeBehaviour.CanDropWeapons()) || (main.currentPvEGameModeBehaviour && main.currentPvEGameModeBehaviour.CanDropWeapons()))
                {
            ...
            public override void DropWeapon(Kit_PlayerBehaviour pb, int slot, Transform replace)
            {
                if (pb.weaponManagerNetworkData != null && pb.weaponManagerNetworkData.GetType() == typeof(Kit_ModernWeaponManagerNetworkData))
                {
                    //Get the manager's runtime data
            ...
            public override void PluginSelectWeapon(Kit_PlayerBehaviour pb, Kit_WeaponBase theWeapon, bool locked = true)
            {
                Kit_ModernWeaponManagerNetworkData runtimeData = pb.weaponManagerNetworkData as Kit_ModernWeaponManagerNetworkData;

                for (int i = 0; i < runtimeData.weaponsInUseDataObjects.Length; i++)
                {
                    if (runtimeData.weaponsInUseDataObjects[i].id == theWeapon.gameGunID)
                    {
                        int slot = i;
                        //Just set, thats all!
                        runtimeData.desiredWeapon = slot;
            ...
            public override void OnTriggerEnterRelay(Kit_PlayerBehaviour pb, Collider col)
            {
                ...
                for (int i = 0; i < runtimeData.weaponsInUseDataObjects.Length; i++)
                {
                    if (runtimeData.weaponsInUseDataObjects[i].behaviour)
                    {
                        //Relay
            ...
            public override void OnDestroyRelay(Kit_PlayerBehaviour pb)
            {
                ...
                for (int i = 0; i < runtimeData.weaponsInUseDataObjects.Length; i++)
                {
                    ...
                    {
                        //Relay
                        runtimeData.weaponsInUseDataObjects[i].behaviour.OnDestroyRelay(pb, runtimeData.weaponsInUseDataObjects[i]);
                    }
                }
            }
        }
