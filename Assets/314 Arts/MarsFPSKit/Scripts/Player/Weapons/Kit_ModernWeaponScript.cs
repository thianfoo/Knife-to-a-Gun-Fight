    ...
    namespace Weapons
    {
        ...
        public class Kit_ModernWeaponScript : Kit_WeaponBase
        {
            ...
            public override WeaponDisplayData GetWeaponDisplayData(Kit_PlayerBehaviour pb, Kit_WeaponRuntimeDataBase runtimeData)
            {
                ...
                wdd.name = weaponName.GetLocalizedString();
                return wdd;
            }
            ...
            public override void SetupFirstPerson(Kit_PlayerBehaviour pb, Kit_WeaponRuntimeDataBase runtimeData)
            {
                ...
                if (data.weaponRenderer.playerModelDependentArmsEnabled && pb.thirdPersonPlayerModel.firstPersonArmsPrefab.Contains(data.weaponRenderer.playerModelDependentArmsKey))
                {
                    ...
                    if (fpa.cameraBoneTargetOverride)
                    {
                        ...
                        {
                            DebugLogError("FPV2 is enabled but first person prefab does not have FPV_SM_Object assigned!");
                        }
                        ...
                        {
                            DebugLogError("FPV3 is enabled but first person prefab does not have FPV_Object assigned!");
                        }
#endif
                    ...
                    for (int i = 0; i < fpa.reparents.Length; i++)
                    {
                        ...
                        {
                            DebugLogWarning("First Person Arm Renderer at index " + i + " is not assigned", fpa);
                        }
                    }
                    //Merge Array
                ...
                main.objectPooling.EnqueueInstantiateable(data.bulletPrefab, 50);
                //And shell
                if (shellEjectionPrefab)
                {
                    main.objectPooling.EnqueueInstantiateable(shellEjectionPrefab, 50);
                }
            }
            ...
            public override void AuthorativeInput(Kit_PlayerBehaviour pb, Kit_WeaponRuntimeDataBase runtimeData, Kit_PlayerInput input, float delta, double revertTime)
            {
                if (runtimeData != null && runtimeData.GetType() == typeof(Kit_ModernWeaponScriptRuntimeData))
                {
                    ...
                    if (!data.reloadInProgress)
                    {
                                ...
                                if (data.lastLmb != input.lmb)
                                {
                                    ...
                                    if (input.lmb)
                                    {
                                        ...
                                        if (Time.time >= data.lastFire + data.fireRate)
                                        {
                                            ...
                                            else if (!main.gameInformation.enableAutoReload || data.bulletsLeftToReload == 0) //No ammo, dry fire
                                            {
                                                DryFire(pb, data);
                                            }
                                        }
                        ...
                        //Check for input
                        if (data.lastReload != input.reload || main.gameInformation.enableAutoReload && data.bulletsLeft == 0 && data.bulletsLeftToReload > 0 && Time.time > data.lastFire + 0.5f)
                        {
                            data.lastReload = input.reload;
                            if (input.reload || main.gameInformation.enableAutoReload && data.bulletsLeft == 0 && data.bulletsLeftToReload > 0)
                            {
                                ...
                                if (data.fireMode != FireMode.BoltAction || data.fireMode == FireMode.BoltAction && Time.time > data.lastFire)
                                {
                                    ...
                                    if (data.bulletsLeftToReload > 0)
                                    {
                                        ...
                                        if ((reloadMode == ReloadMode.Chambered || reloadMode == ReloadMode.ProceduralChambered) && data.bulletsLeft != data.bulletsPerMag + 1 || reloadMode != ReloadMode.Chambered && reloadMode != ReloadMode.ProceduralChambered && data.bulletsLeft != data.bulletsPerMag)
                                        {
                                            if (pb.isLocalPlayer)
                                            {
                                                pb.inputManager.ResetAiming(pb);
                                            }
                            ...
                            switch (reloadMode)
                            {
                                ...
                                case ReloadMode.Simple:
                                    if (data.reloadPhase == 0)
                                    {
                                        ...
                                        {
                                            if (!main.gameInformation.debugEnableUnlimitedReloads)
                    ...
                    if (pb.isFirstPersonActive)
                    {
                        if (data.weaponRenderer.cachedUseFullscreenScope)
                        {
                            if (data.isAimedIn)
                            {
                                ...
                                main.hud.DisplaySniperScope(true);
                                //Snap fov
                                main.cameraManager.mainCamera.fieldOfView = data.weaponRenderer.cachedAimingFov;
                                //Hide weapon
                            ...
                            else
                            {
                                ...
                                main.hud.DisplaySniperScope(false);
                                //Smoothly fade FoV
                                main.cameraManager.mainCamera.fieldOfView = Mathf.Lerp(Kit_GameSettings.baseFov, Kit_GameSettings.baseFov - 5f, data.aimingProgress);
                                //Show weapon
                                if (data.sniperWeaponHidden)
                                {
                        ...
                        else
                        {
                            ...
                            main.hud.DisplaySniperScope(false);
                            //Smoothly fade FoV
                            main.cameraManager.mainCamera.fieldOfView = Mathf.Lerp(Kit_GameSettings.baseFov, data.weaponRenderer.cachedAimingFov, data.aimingProgress);
                        }
                    ...
                    {
                        //Update HUD
                        main.hud.DisplayAmmo(data.bulletsLeft, data.bulletsLeftToReload);
                        main.hud.DisplayCrosshair(GetCrosshairSize(pb, data), pb.looking.GetPerspective(pb) == Kit_GameInformation.Perspective.ThirdPerson);
                    }
                }
            }
            ...
            public override void FallDownEffect(Kit_PlayerBehaviour pb, Kit_WeaponRuntimeDataBase runtimeData, bool wasFallDamageApplied)
            {
                if (runtimeData != null && runtimeData.GetType() == typeof(Kit_ModernWeaponScriptRuntimeData))
                {
                    ...
                    {
                        coroutineHelper.StartCoroutine(coroutineHelper.Kick(data.weaponFallTransform, new Vector3(fallDownAmount, Random.Range(fallDownMinOffset, fallDownMaxoffset), 0), fallDownTime));
                    }
                    else
                    {
                        coroutineHelper.StartCoroutine(coroutineHelper.Kick(data.weaponFallTransform, new Vector3(fallDownAmount / 3, Random.Range(fallDownMinOffset, fallDownMaxoffset) / 2, 0), fallDownTime));
                    }
                }
            }
            ...
            public override void PutawayWeapon(Kit_PlayerBehaviour pb, Kit_WeaponRuntimeDataBase runtimeData)
            {
                if (runtimeData != null && runtimeData.GetType() == typeof(Kit_ModernWeaponScriptRuntimeData))
                {
                    ...
                    if (pb.isFirstPersonActive)
                    {
                        ...
                        {
                            //Play animation
                            data.weaponRenderer.anim.CrossFade("Putaway", 0.2f, data.weaponRenderer.animActionLayer);
            ...
            public void FireOneShot(Kit_PlayerBehaviour pb, Kit_ModernWeaponScriptRuntimeData data, float delta, float revertBy, bool rpc)
            {
                ...
                //Subtract bullets
                if (!main.gameInformation.debugEnableUnlimitedBullets)
                    data.bulletsLeft--;
                ...
                coroutineHelper.StartCoroutine(coroutineHelper.WeaponApplyRecoil(this, data, pb, RandomExtensions.RandomBetweenVector2(data.recoilPerShotMin, data.recoilPerShotMax), data.recoilApplyTime));
            }
            ...
            void FireBurst(Kit_PlayerBehaviour pb, Kit_ModernWeaponScriptRuntimeData data, float delta, float revertBy, bool rpc)
            {
                ...
                //Start Coroutine
                coroutineHelper.StartCoroutine(coroutineHelper.WeaponBurstFire(this, data, pb, delta, revertBy, rpc));
            }
            ...
            public void FireRaycast(Kit_PlayerBehaviour pb, Kit_ModernWeaponScriptRuntimeData data, float delta, float revertBy)
            {
                if (pb.isLocalPlayer || pb.isServer)
                {
                    if (data.bulletsPenetrationEnabled)
                    {
                        ...
                        if (pb.isFirstPersonActive && main.gameInformation.thirdPersonCameraShooting && pb.looking.GetPerspective(pb) == Kit_GameInformation.Perspective.ThirdPerson)
                        {
                            dir = main.mainCamera.transform.forward + GetSpread(pb, data);
                            pos = main.mainCamera.transform.position;
                        }
                        */

                        RaycastHit[] hits = Physics.RaycastAll(pos, dir, data.range, pb.gameInformation.weaponHitLayers.value).OrderBy(h => h.distance).ToArray();
                        for (int i = 0; i < hits.Length && continueLoop; i++)
                        {
                            ...
                            if (hit.collider.gameObject.transform.root != pb.transform.root)
                            {
                                ...
                                if (hit.collider.GetComponent<Kit_PlayerDamageMultiplier>() && main.currentGameModeType == 2)
                                {
                                    if (pb.isLocalPlayer || pb.isServer && pb.isBot)
                                    {
                                        ...
                                        if (hit.collider.transform.root.GetComponent<Kit_PlayerBehaviour>())
                                        {
                                            ...
                                            if (!damagedPlayers.Contains(hitPb))
                                            {
                                                ...
                                                if (pb.isBot)
                                                {
                                                    hitPb.ServerDamage(GetDamage(data, Vector3.Distance(pb.playerCameraTransform.position, hit.point)) * pdm.damageMultiplier, gameGunID, pb.transform.position, dir, ragdollForce, hit.point, pdm.ragdollId, pb.isBot, pb.id);
                                                }
                                                else
                                                {
                                                    //Tell server
                                                    data.FiredWeapon(hitPb, pdm.ragdollId, pos, hit.point);
                                                }
                                            }
                                        }
                                    }
                                ...
                                else if (hit.collider.GetComponent<IKitDamageable>() != null)
                                {
                                    IKitDamageable damage = hit.collider.GetComponent<IKitDamageable>();

                                    if (pb.isLocalPlayer || pb.isServer && pb.isBot)
                                    {
                                        if (damage != null)
                                        {
                                            if (damage.LocalDamage(GetDamage(data, hit.distance), gameGunID, pos, dir, ragdollForce, hit.point, pb.isBot, pb.id))
                                            {
                                                if (pb.isBot)
                                                {
                                                    damage.ServerDamage(GetDamage(data, Vector3.Distance(pb.playerCameraTransform.position, hit.point)), gameGunID, pb.transform.position, dir, ragdollForce, hit.point, pb.isBot, pb.id);
                                                }
                                                else
                                                {
                                                    var lagCompensator = damage.GetLagCompensator();
                                                    data.FiredWeapon(lagCompensator, pos, hit.point);
                                                }
                                            }
                                        }
                                    }
                    ...
                    else
                    {
                        ...
                        */
                        ...
                        {
                            hit = hits[i];
                            DebugLog(hit.collider.gameObject.name, hit.collider);
                            //Check if we hit ourselves
                            if (hit.collider.gameObject.transform.root != pb.transform.root)
                            {
                                //Check if we hit a player
                    ...
                    if (main.minimap)
                    {
                        main.minimap.PlayerShoots(pb, data.tpWeaponRenderer.isWeaponSilenced);
                    }
                }
            }
            ...
            public override void NetworkPhysicalBulletFired(Kit_PlayerBehaviour pb, Vector3 pos, Vector3 dir, Kit_WeaponRuntimeDataBase runtimeData)
            {
                if (runtimeData != null && runtimeData.GetType() == typeof(Kit_ModernWeaponScriptRuntimeData))
                {
                    ...
                    GameObject bullet = main.objectPooling.GetInstantiateable(data.bulletPrefab, pos, Quaternion.LookRotation(dir));
                    //Call setup
            ...
            {
                if (/*main.gameInformation.fireShotsLocally ||*/ pb.isServer)
                {
                    Vector3 dir = pb.playerCameraTransform.forward + GetSpread(pb, data);
                    Vector3 pos = pb.playerCameraTransform.position;
            ...
            /// <returns></returns>
            public float GetDamage(Kit_ModernWeaponScriptRuntimeData data, float distance)
            {
            ...
            void EjectShell(Kit_PlayerBehaviour pb, Kit_ModernWeaponScriptRuntimeData data)
            {
                if (shellEjectionPrefab & data.weaponRenderer.shellEjectTransform)
                {
                    ...
                    if (pb.looking.GetPerspective(pb) == Kit_GameInformation.Perspective.FirstPerson)
                    {
                        shellObj = main.objectPooling.GetInstantiateable(shellEjectionPrefab, data.weaponRenderer.shellEjectTransform.position, data.weaponRenderer.shellEjectTransform.rotation);
                    }
                    else
                    {
                        shellObj = main.objectPooling.GetInstantiateable(shellEjectionPrefab, data.tpWeaponRenderer.shellEjectTransform.position, data.weaponRenderer.shellEjectTransform.rotation);
                    }
                    if (!shellObj) return;
                    //Apply force
