...
{
    public class Kit_ThirdPersonWeaponRenderer : Kit_Base
    {
        /// <summary>
        ...
        void OnEnable()
        {
            for (int i = 0; i < allWeaponRenderers.Length; i++)
            {
                ...
                {
                    DebugLogError("Third person weapon renderer from " + gameObject.name + " at index " + i + " not assigned.");
                }
            }
        }
#endif
        ...
        public void SetAttachments(int[] enabledAttachments, Kit_ModernWeaponScript ws, Kit_PlayerBehaviour pb, Kit_ModernWeaponScriptRuntimeData data)
        {
            ...
            for (int i = 0; i < enabledAttachments.Length; i++)
            {
                ...
                else if (ws.attachmentSlots[i].availableAttachments[enabledAttachments[i]].GetType() == typeof(Kit_SkinInfo))
                {
                    ...
                    {
                        if (skin.setToRendererWithIndexTpOverride.Length != attachmentAndSkinSockets[i].renderersToChange.Length) DebugLogWarning("Skin material length and the amount of renderers in slot #" + i + " do not match up. Trying best to assign.");
            ...
            if (data != null && data.weaponRenderer)
            {
                ...
                {
                    DebugLogWarning("FP / TP  Attachment sync lists don't match! Check your attachment's TP Override script");
                }
            }
        }
    }
