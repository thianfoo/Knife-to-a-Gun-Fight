...
namespace MarsFPSKit
{
    namespace Weapons
    {
        public class Kit_ModernWeaponManagerNetworkData : Kit_WeaponManagerNetworkBase
        {
            ...
            private void Start()
            {
                weaponsInUseSync.OnChange += OnWeaponsInUseSyncCallback;
            }

            private void OnWeaponsInUseSyncCallback(SyncList<uint>.Operation operation, int index, uint item)
            {
                ...
                if (ownerPlayer)
                {
                    ownerPlayer.weaponManager.OnWeaponsInUseSyncCallback(ownerPlayer, operation, index, item);
                }
            }
            ...
            public Kit_WeaponRuntimeDataBase GetWeapon(int index)
            {
                ...
                {
                    weaponsInUseDataObjects = new Kit_WeaponRuntimeDataBase[weaponsInUseSync.Count];
                    ...
                    for (int i = 0; i < weaponsInUseSync.Count; i++)
                    {
                        ...
                        if (id)
                        {
                            weaponsInUseDataObjects[saved_i] = id.GetComponent<Kit_WeaponRuntimeDataBase>();
                        }
                        else
                        {
                            //Didn't find it. This shouldn't happen!
                            weaponsInUseDataObjects[saved_i] = null;
                        }
                    }
                }
