    ...
    public class Kit_RigidbodyBullet : Kit_BulletBase
    {
        ...
        void OnCollisionEnter(Collision collision)
        {
            ...
            if (onImpact == RigidbodyBulletOnImpact.Impact)
            {
                if (collision.contacts[0].otherCollider.transform.GetComponent<Kit_PlayerDamageMultiplier>() && main.currentGameModeType == 2)
                {
                    ...
                    if (collision.contacts[0].otherCollider.transform.root.GetComponent<Kit_PlayerBehaviour>())
                    {
                        if (settings.isServer)
                        {
                            ...
                            if ((settings.localOwner && main.currentPvPGameModeBehaviour.ArePlayersEnemies(settings.localOwner, hitPb)) || (!settings.localOwner && main.currentPvPGameModeBehaviour.ArePlayersEnemies(settings.ownerID, settings.ownerIsBot, hitPb, false)))
                            {
                ...
                {
                    //Call
                    main.ClientImpactProcess(collision.contacts[0].point, collision.contacts[0].normal, collision.contacts[0].otherCollider.tag);
                }
            }
        ...
        private void OnTriggerEnter(Collider other)
        {
            ...
            if (onImpact == RigidbodyBulletOnImpact.Impact)
            {
                ...
                if (other.transform.GetComponent<Kit_PlayerDamageMultiplier>() && main.currentGameModeType == 2)
                {
                    ...
                    {
                        //Call
                        main.ClientImpactProcess(transform.position, transform.forward, other.tag);
                    }
                }
            }
        }
