        ...
        public class Kit_ModernMeleeScript : Kit_WeaponBase
        {
                ...
                if (primaryAttack == AttackType.Stab)
                {
                    ...
                    if (data.lastLmb != pb.input.lmb)
                    {
                        ...
                        if (pb.input.lmb)
                        {
                            ...
                            {
                                coroutineHelper.StartCoroutine(coroutineHelper.MeleeExecuteStab(this, data, primaryAttackSettings, pb, (float)revertTime));
                            }
                        }
                    }
                }
                else if (primaryAttack == AttackType.Charge)
                {
                    if (!data.chargingSecondary)
                    {
                        ...
                        else
                        {
                            ...
                            {
                                //RELEASE...
                                coroutineHelper.StartCoroutine(coroutineHelper.MeleeExecuteCharge(this, data, primaryAttackSettings, pb, (float)revertTime));
                                //Reset bools
                ...
                else if (primaryAttack == AttackType.Heal)
                {
                    ...
                    if (data.lastLmb != pb.input.lmb)
                    {
                        ...
                        if (pb.input.lmb)
                        {
                            ...
                            {
                                coroutineHelper.StartCoroutine(coroutineHelper.MeleeExecuteHeal(this, data, primaryAttackSettings, pb, (float)revertTime));
                            }
                        }
                    }
                }
                        ...
                        if (pb.input.lmb)
                        {
                            if (Time.time > data.nextActionPossibleAt && !data.isCharging)
                            {
                                if (primaryAttackSettings.combos.Length > 0)
                                {
                                    ...
                                    coroutineHelper.StartCoroutine(coroutineHelper.MeleeExecuteCombo(this, data, primaryAttackSettings, true, pb, (float)revertTime));
                                }
                                else
                                {
                                    DebugLogWarning("No combos for primary attack specified!");
                                }
                            }
                        }
                    }
                ...
                if (secondaryAttack == AttackType.Stab)
                {
                    ...
                    if (data.lastRmb != pb.input.rmb)
                    {
                        ...
                        if (pb.input.rmb)
                        {
                            if (Time.time > data.nextActionPossibleAt && !data.isCharging)
                            {
                                coroutineHelper.StartCoroutine(coroutineHelper.MeleeExecuteStab(this, data, secondaryAttackSettings, pb, (float)revertTime));
                            }
                        }
                    }
                }
                else if (secondaryAttack == AttackType.Charge)
                {
                    if (!data.chargingPrimary)
                    {
                        ...
                        else
                        {
                            ...
                            {
                                //RELEASE...
                                coroutineHelper.StartCoroutine(coroutineHelper.MeleeExecuteCharge(this, data, secondaryAttackSettings, pb, (float)revertTime));
                                //Reset bools
                                data.isCharging = false;
                ...
                else if (secondaryAttack == AttackType.Heal)
                {
                    ...
                    if (data.lastRmb != pb.input.rmb)
                    {
                        ...
                        if (pb.input.rmb)
                        {
                            ...
                            {
                                coroutineHelper.StartCoroutine(coroutineHelper.MeleeExecuteHeal(this, data, secondaryAttackSettings, pb, (float)revertTime));
                            }
                        }
                    }
                }
                        ...
                        if (pb.input.lmb)
                        {
                            if (Time.time > data.nextActionPossibleAt && !data.isCharging)
                            {
                                if (secondaryAttackSettings.combos.Length > 0)
                                {
                                    ...
                                    coroutineHelper.StartCoroutine(coroutineHelper.MeleeExecuteCombo(this, data, secondaryAttackSettings, false, pb, (float)revertTime));
                                }
                                else
                                {
                                    DebugLogWarning("No combos for secondary attack specified!");
                                }
                            }
                        }
                    }
            ...
            public override float BeginQuickUse(Kit_PlayerBehaviour pb, Kit_WeaponRuntimeDataBase runtimeData, float revertBy)
            {
                if (runtimeData != null && runtimeData.GetType() == typeof(Kit_ModernMeleeScriptRuntimeData))
                {
                    ...
                    {
                        //Start Coroutine!
                        coroutineHelper.StartCoroutine(coroutineHelper.MeleeExecuteStab(this, data, quickAttackSettings, pb, revertBy));
                    ...
                    {
                        //Start Coroutine!
                        coroutineHelper.StartCoroutine(coroutineHelper.MeleeExecuteHeal(this, data, quickAttackSettings, pb, revertBy));
            ...
            public override float EndQuickUse(Kit_PlayerBehaviour pb, Kit_WeaponRuntimeDataBase runtimeData, float revertBy)
            {
                if (runtimeData != null && runtimeData.GetType() == typeof(Kit_ModernMeleeScriptRuntimeData))
                {
                    ...
                    {
                        //Finish him.
                        coroutineHelper.StartCoroutine(coroutineHelper.MeleeExecuteCharge(this, data, quickAttackSettings, pb, revertBy));
            ...
            public override void CalculateWeaponUpdate(Kit_PlayerBehaviour pb, Kit_WeaponRuntimeDataBase runtimeData)
            {
                if (runtimeData != null && runtimeData.GetType() == typeof(Kit_ModernMeleeScriptRuntimeData))
                {
                    ...
                    if (pb.isFirstPersonActive)
                    {
                        ...
                        {
                            main.hud.DisplayAmmo(data.primaryHealingAmount, data.secondaryHealingAmount);
                        }
                        else if (primaryAttack == AttackType.Heal)
                        {
                            main.hud.DisplayAmmo(data.primaryHealingAmount, 0);
                        }
                        else if (secondaryAttack == AttackType.Heal)
                        {
                            main.hud.DisplayAmmo(0, data.secondaryHealingAmount);
                        }
                        else
                        {
                            main.hud.DisplayAmmo(0, 0, false);
                        }
                    }
                }
            }
