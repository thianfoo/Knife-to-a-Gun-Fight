...
namespace MarsFPSKit
{
    namespace Weapons
    {
        public class Kit_ModernWeaponScriptRuntimeData : Kit_WeaponRuntimeDataBase
        {
            ...
            public void FiredWeapon(Kit_PlayerBehaviour target, int damageIndex, Vector3 originPoint, Vector3 hitPoint)
            {
                if (!pb.isBot)
                {
                    CmdFiredWeapon(target, damageIndex, originPoint, hitPoint);
                }
            }

            [Command]
            public void CmdFiredWeapon(Kit_PlayerBehaviour target, int damageIndex, Vector3 originPoint, Vector3 hitPoint)
            {
                FiredWeaponInternal(target, damageIndex, originPoint, hitPoint);
            }

            private void FiredWeaponInternal(Kit_PlayerBehaviour target, int damageIndex, Vector3 originPoint, Vector3 hitPoint)
            {
                if (!pb.isBot)
                {
                    if (target)
                    {
                        ...
                        if (compensator)
                        {
                            ...
                            Vector3 dir = (hitPoint - originPoint).normalized;
                            ...
                            if (compensator.RaycastCheck(this.connectionToClient, originPoint, hitPoint, pb.gameInformation.lagCompensationTolerancePercent, target.gameInformation.weaponHitLayersNoPlayers, out hit))
                            {
                                ...
                                {
                                    Kit_PlayerDamageMultiplier pdm = target.thirdPersonPlayerModel.damageMultipliers[damageIndex];
                                    //if (hit.collider.transform.root.GetComponent<Kit_PlayerBehaviour>())
                                    {
                                        //First check if we can actually damage that player
                                        if (main.currentPvPGameModeBehaviour.ArePlayersEnemies(pb, target))
                                        {
                                            //Check if he has spawn protection
                                            if (!target.spawnProtection || target.spawnProtection.CanTakeDamage(target))
                                            {
                                                //Apply local damage, sample damage dropoff via distance
                                                if (pb.isServer)
                                                {
                                                    target.ServerDamage((behaviour as Kit_ModernWeaponScript).GetDamage(this, Vector3.Distance(pb.playerCameraTransform.position, hit.point)) * pdm.damageMultiplier, id, pb.transform.position, dir, ragdollForce, hit.point, pdm.ragdollId, pb.isBot, pb.id);
            ...
            public void FiredWeapon(LagCompensator target, Vector3 originPoint, Vector3 hitPoint)
            {
                if (!pb.isBot)
                {
                    CmdFiredWeapon(target, originPoint, hitPoint);
                }
            }

            [Command]
            public void CmdFiredWeapon(LagCompensator target, Vector3 originPoint, Vector3 hitPoint)
            {
                FiredWeaponInternal(target, originPoint, hitPoint);
            }

            private void FiredWeaponInternal(LagCompensator target, Vector3 originPoint, Vector3 hitPoint)
            {
                if (!pb.isBot)
                {
                    if (target)
                    {
                        ...
                        if (compensator)
                        {
                            ...
                            // viewer is the one who fired, so it's 'this' object.
                            // Lag Compensator checks 'this' object's latency and then rewinds 'target's history by that time.
                            if (compensator.RaycastCheck(this.connectionToClient, originPoint, hitPoint, pb.gameInformation.lagCompensationTolerancePercent, pb.gameInformation.weaponHitLayersNoPlayers, out hit))
                            {
                                //Apply local damage, sample damage dropoff via distance
                                if (pb.isServer)
                                {
                                    var dmg = target.GetComponentInChildren<IKitDamageable>();
                                    if (dmg != null)
                                    {
                                        float damage = (behaviour as Kit_ModernWeaponScript).GetDamage(this, Vector3.Distance(pb.playerCameraTransform.position, hit.point));
                                        dmg.ServerDamage(damage, id, pb.playerCameraTransform.position, dir, ragdollForce, hit.point, pb.isBot, pb.id);
                                        //Show hitmarker
                                        pb.TargetHitMarker(0);
                                    }
                                }
                            }
                        }
