...
namespace MarsFPSKit.Weapons
{
    ...
    public class Kit_WeaponRenderer : Kit_Base
    {
        /// <summary>
        /// The weapon animator
        /// </summary>
        ...
        void OnEnable()
        {
            for (int i = 0; i < allWeaponRenderers.Length; i++)
            {
                ...
                {
                    DebugLogError("Weapon renderer from " + gameObject.name + " at index " + i + " not assigned.");
                }
            }
        }
#endif
                            ...
                            for (int p = 0; p < attachmentVisualBehaviours.Length; p++)
                            {
                                ...
                                else if (attachmentVisualBehaviours[p].GetType() == typeof(Kit_AttachmentRenderer))
                                {
                                    ...
                                    if (pb)
                                    {
#if INTEGRATION_FPV2
                                            //Set shaders
                                            FirstPersonView.ShaderMaterialSolution.FPV_SM_Object obj = ar.GetComponent<FirstPersonView.ShaderMaterialSolution.FPV_SM_Object>();
                                            if (obj)
                                            {
                                            ...
                                            {
                                                DebugLogError("FPV2 is enabled but attachment first person prefab does not have FPV_SM_Object assigned!");
                                            }
#elif INTEGRATION_FPV3
                                            //Set shaders
                                            FirstPersonView.FPV_Object obj = ar.GetComponent<FirstPersonView.FPV_Object>();
                                            if (obj)
                                            {
                                                //Set as first person object
                                                obj.SetAsFirstPersonObject();
                                            ...
                                            {
                                                DebugLogError("FPV3 is enabled but attachment first person prefab does not have FPV_Object assigned!");
                                            }
#endif
                                    }
                                }
