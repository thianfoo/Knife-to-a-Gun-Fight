...
namespace MarsFPSKit
{
    ...
    public class Kit_Movement_BootsOnGround : Kit_MovementBase
    {
        ...
        public float lerpDistance = 0.06f;
        ...
        public override void InitializeClient(Kit_PlayerBehaviour pb)
        {
            ...
            {
                //Find ours
                pb.movementNetworkData = FindObjectsByType<Kit_Movement_BootsOnGroundNetworkData>(FindObjectsSortMode.None).Where(x => x.ownerPlayerNetworkId == pb.netId).FirstOrDefault();
            }
        ...
        public override void NotControllerUpdate(Kit_PlayerBehaviour pb)
        {
            ...
            //Manual sync. Very simple, but it always works.
            pb.transform.position = Vector3.Lerp(pb.transform.position, bogrd.serverPosition, Time.deltaTime * 10f);
            pb.transform.rotation = Quaternion.Slerp(pb.transform.rotation, bogrd.serverRotation, Time.deltaTime * 10f);

            if (!pb.isFirstPersonActive)
                bogrd.smoothedVelocity = Vector3.Lerp(bogrd.smoothedVelocity, bogrd.velocity, Time.deltaTime * 10);
        }
        ...
        public override void AuthorativeInput(Kit_PlayerBehaviour pb, Kit_PlayerInput input, float delta, double revertTime)
        {
            ...
            if (data.isSwimming)
            {
                ...
                data.smoothedVelocity = data.velocity;
                data.localMovementVelocityNormalized = data.localMoveDirection.normalized;

                if (pb.isServer)
                {
                    data.netLocalMovementVelocityNormalized = data.localMovementVelocityNormalized;
                }
                #endregion
                ...
                if (pb.cc.isGrounded)
                {
                    ...
                    if (data.falling)
                    {
                        ...
                        if (data.fallDistance > fallDamageThreshold)
                        {
                            ...
                            if (pb.isFirstPersonActive)
                            {
                                coroutineHelper.StartCoroutine(coroutineHelper.Kick(pb.playerCameraFallDownTransform, new Vector3(fallDownAmount, Random.Range(fallDownMinOffset, fallDownMaxoffset), 0), fallDownTime));
                                //Tell weapon manager
                        ...
                        else if (data.fallDistance > 0.1f)
                        {
                            if (pb.isFirstPersonActive)
                            {
                                coroutineHelper.StartCoroutine(coroutineHelper.Kick(pb.playerCameraFallDownTransform, new Vector3(fallDownAmount / 3, Random.Range(fallDownMinOffset, fallDownMaxoffset) / 2, 0), fallDownTime));
                                //Tell weapon manager
            ...
            data.moving = (data.velocity.magnitude > 0.5f);
        ...
        public override void PredictionInput(Kit_PlayerBehaviour pb, Kit_PlayerInput input, float delta)
        {
            ...
            if (!pb.isServer)
            {
                if (data.historicalPositions.Count > 0)
                {
                    ...
                    //DebugLog("Calculated snapshot: " + snapShotToUse);
                    //DebugLog("Closest snapshot: " + closestSnapshot);
        ...
        public override void Visuals(Kit_PlayerBehaviour pb)
        {
            ...
            if (pb.isFirstPersonActive)
            {
                ...
                {
                    main.hud.DisplayStamina(data.airLeft);
                }
                else
                {
                    //Display stamina
                    main.hud.DisplayStamina(data.staminaLeft);
                }

                if (data.state == 0)
                {
                    main.hud.DisplayMovementState(0);
                }
                else if (data.state == 1)
                {
                    main.hud.DisplayMovementState(1);
                }
            }
        }
        ...
        public override void OnControllerColliderHitRelay(Kit_PlayerBehaviour pb, ControllerColliderHit hit)
        {
            ...
            if (terrain && convertedFootsteps)
            {
                ...
                {
                    DebugLogError("Terrain texture outside of texture to foosteps conversion array! Playing concrete footstep.");
                    bogrd.currentMaterial = "Concrete";
                }
            }
            else
        ...
        public override Vector3 GetVelocity(Kit_PlayerBehaviour pb)
        {
            ...
            {
                return bogrd.smoothedVelocity;
            }
        }

        public override Vector3 GetLocalVelocity(Kit_PlayerBehaviour pb)
        {
            Kit_Movement_BootsOnGroundNetworkData bogrd = pb.movementNetworkData as Kit_Movement_BootsOnGroundNetworkData;
            ...
            {
                //If we are the controller, just get velocity from the character controller
                return pb.transform.InverseTransformDirection(pb.cc.velocity);
            }
            else
            {
                //If we are the controller, just get velocity from the character controller
                return pb.transform.InverseTransformDirection(bogrd.smoothedVelocity);
            }
        }
        ...
        public override void OnTriggerEnterRelay(Kit_PlayerBehaviour pb, Collider col)
        {
            ...
            if (col.GetComponent<Kit_SwimmingTrigger>())
            {
                ...
                if (pb.isServer)
                {
                    ...
                    {
                        Kit_Movement_BootsOnGroundNetworkData bogrd = (Kit_Movement_BootsOnGroundNetworkData)pb.movementNetworkData;
                        DebugLog("[Player] Entered swimming trigger. Entering swim state.");
                        ...
                        if (swimmingWeapon)
                        {
                            ...
                            pb.weaponManager.PluginSelectWeapon(pb, swimmingWeapon, true);
                        }
                    }
                }
                else
            ...
            if (swim)
            {
                ...
                if (pb.isServer)
                {
                    if (pb.movementNetworkData != null && pb.movementNetworkData.GetType() == typeof(Kit_Movement_BootsOnGroundNetworkData))
                    {
                        ...
                        {
                            DebugLog("[Player] Exited swimming trigger. Exiting swim state.");
                            //Set bool
        ...
        public override void OnCameraTriggerEnterRelay(Kit_PlayerBehaviour pb, Collider col)
        {
            ...
            if (swim)
            {
                ...
                if (bogrd.swimmingCurrent == swim)
                {
                    ...
                    {
                        //Show
                        main.hud.DisplayUnderwater(true);
                    }
                }
            }
        }
        ...
        public override void OnCameraTriggerExitRelay(Kit_PlayerBehaviour pb, Collider col)
        {
            ...
            if (swim)
            {
                ...
                if (bogrd.swimmingCurrent == swim)
                {
                    ...
                    {
                        //Hide
                        main.hud.DisplayUnderwater(false);
                    }
                }
            }
        }
        ...
        public override WeaponsFromPlugin WeaponsToInjectIntoWeaponManager()
        {
            if (swimmingWeapon)
            {
            ...
            {
                return base.WeaponsToInjectIntoWeaponManager();
            }
        }
        #endregion
