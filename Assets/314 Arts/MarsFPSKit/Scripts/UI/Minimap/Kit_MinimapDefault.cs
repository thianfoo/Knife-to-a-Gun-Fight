...
namespace MarsFPSKit
{
    ...
    public class Kit_MinimapDefault : Kit_MinimapBase
    {
        ...
        [ClientRpc]
        public void PlayerShot(uint netId)
        {
            if (NetworkClient.active)
            {
                if (NetworkClient.spawned.ContainsKey(netId))
                {
                    NetworkIdentity networkIdentity = NetworkClient.spawned[netId];
                    Kit_PlayerBehaviour pb = networkIdentity.GetComponent<Kit_PlayerBehaviour>();
                    if (pb)
                    {
                        if (activePlayers.ContainsKey(pb))
                        {
                            //Check if we're enemies
                            if (main.currentPvPGameModeBehaviour && main.currentPvPGameModeBehaviour.AreWeEnemies(pb.isBot, pb.id))
                            {
