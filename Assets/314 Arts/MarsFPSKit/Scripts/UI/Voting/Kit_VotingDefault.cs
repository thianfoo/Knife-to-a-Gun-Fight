...
namespace MarsFPSKit
{
    public class Kit_VotingDefault : Kit_VotingBase
    {
        ...
        {
            networkPlayerManager.onPlayerLeft.AddListener(OnPlayerLeft);
        }

        private void OnDisable()
        {
            networkPlayerManager.onPlayerLeft.RemoveListener(OnPlayerLeft);
        }

        void Start()
        {
            playersToVote.OnChange += OnVotesChanged;
        }
        ...
        void Update()
        {
            ...
            if (myVote == 0)
            {
                if (inputSystem.voteYes)
                {
                    VoteYes();
                }
                else if (inputSystem.voteNo)
                {
                    VoteNo();
                }
            }
        ...
        void RecalculateVotes()
        {
            if (isServer)
            {
                ...
                if ((GetYesVotes() + GetNoVotes()) >= networkPlayerManager.players.Count)
                {
        ...
        void TimeRanOut()
        {
            if (isServer)
            {
                ...
                if (totalVotes > (networkPlayerManager.players.Count / 2))
                {
                    if (GetYesVotes() > GetNoVotes())
                    {
                        VoteSucceeded();
        ...
        void VoteSucceeded()
        {
            if (isServer)
            {
                ...
                {
                    //Get that player
                    Kit_Player toKick = networkPlayerManager.GetPlayerById((uint)argument);
                    if (toKick != null)
                    {
                        //Kick that player
                ...
                {
                    //Switch map
                    main.SwitchMap((int)argument);
                }
                else if (votingOn == VotingOn.GameMode)
                {
                    //Switch game mode
                    main.SwitchGameMode((int)argument);
                }
