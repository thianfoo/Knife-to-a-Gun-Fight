...
namespace MarsFPSKit.UI
{
    public class Kit_LoadoutNew : Kit_LoadoutBase
    {
        ...
        /// What is shown
        /// </summary>
        public LocalizedString loadoutText;
        /// <summary>
        ...
        /// <summary>
        /// Shows us when that weapon is being unlocked
        /// </summary>
        public LocalizedString weaponSelectionUnlocksAtLevelText;
        /// <summary>
        ...
        private void Awake()
        {
            ...
            for (int i = 0; i < subMenus.Length; i++)
            {
                ...
                {
                    DebugLogError("Submenu root at index " + i + " is not assigned.", this);
                }
            }
        ...
        {
            menu = FindAnyObjectByType<UI.Kit_MenuManager>();
            ...
            for (int i = 0; i < amountOfLoadouts; i++)
            {
                ...
                if (txt) txt.text = string.Format(loadoutText.GetLocalizedString(), (id + 1));
                //Setup Button
            ...
            for (int i = 0; i < allLoadouts.Length; i++)
            {
                ...
                for (int j = 0; j < allLoadouts[id].loadoutWeapons.Length; j++)
                {
                    ...
                    //Set default one
                    allLoadouts[id].loadoutWeapons[jd].weaponID = System.Array.IndexOf(game.allWeapons, game.defaultWeaponsInSlot[jd]);
            ...
            for (int i = 0; i < game.allWeaponCategories.Length; i++)
            {
                ...
                //Setup text
                TextMeshProUGUI txt = go.GetComponentInChildren<TextMeshProUGUI>();
                if (txt) txt.text = game.allWeaponCategories[id].GetLocalizedString();
                //Setup Button
                Button btn = go.GetComponentInChildren<Button>();
            ...
            for (int i = 0; i < game.allPvpTeams.Length; i++)
            {
                ...
                //Setup text
                TextMeshProUGUI txt = go.GetComponentInChildren<TextMeshProUGUI>();
                if (txt) txt.text = game.allPvpTeams[id].teamName.GetLocalizedString();
                //Setup Button
                Button btn = go.GetComponentInChildren<Button>();
        ...
        public void WeaponSelectCategory(int id)
        {
            ...
            for (int i = 0; i < game.allWeapons.Length; i++)
            {
                if (game.allWeapons[i].CanBeSelectedInLoadout())
                {
                    ...
                    if (game.allWeapons[i].weaponType.GetLocalizedString() == game.allWeaponCategories[currentSelectedWeaponCategory].GetLocalizedString())
                    {
                        //Add our sub category if its not added yet
                        if (!subcategories.Contains(game.allWeapons[i].weaponLoadoutSubCategory.GetLocalizedString()))
                        {
                            subcategories.Add(game.allWeapons[i].weaponLoadoutSubCategory.GetLocalizedString());
                        }
                    }
                }
            }
        ...
        public void WeaponSelectSubcategory(int cat, string subCat)
        {
            ...
            for (int i = 0; i < game.allWeapons.Length; i++)
            {
                ...
                {
                    //Check if weapon is for our category
                    if (game.allWeapons[id].weaponType.GetLocalizedString() == game.allWeaponCategories[currentSelectedWeaponCategory].GetLocalizedString())
                    {
                        //Check if weapon is in that sub category
                        if (game.allWeapons[id].weaponLoadoutSubCategory.GetLocalizedString() == subCat)
                        {
                            //Create
                            ...
                            if (txt)
                            {
                                ...
                                {
                                    txt.text = game.allWeapons[id].weaponName.GetLocalizedString();
                                }
                                else
                                {
                                    txt.text = string.Format(weaponSelectionUnlocksAtLevelText.GetLocalizedString(), game.allWeapons[id].weaponName.GetLocalizedString(), game.allWeapons[id].levelToUnlockAt);
                                }
                            }
                            //Setup Button
                            Button btn = go.GetComponentInChildren<Button>();
            ...
            if (game.allWeapons[id].IsWeaponUnlocked(game))
            {
                ...
                if (wpnRender && wpnRender.attachmentSlots.Length > 0)
                {
                    ...
                    for (int i = 0; i < wpnRender.attachmentSlots.Length; i++)
                    {
                        ...
                        //Setup text
                        TextMeshProUGUI txt = go.GetComponentInChildren<TextMeshProUGUI>();
                        if (txt) txt.text = wpnRender.attachmentSlots[catId].slotName.GetLocalizedString();
                        //Setup Button
                        Button btn = go.GetComponentInChildren<Button>();
        ...
        public void WeaponCustomizationSelectCategory(int slot, int wep, int catId)
        {
            ...
            if (wpnRender)
            {
                for (int i = 0; i < wpnRender.attachmentSlots[catId].availableAttachments.Length; i++)
                {
                    ...
                    //Setup text
                    TextMeshProUGUI txt = go.GetComponentInChildren<TextMeshProUGUI>();
                    if (txt) txt.text = wpnRender.attachmentSlots[catId].availableAttachments[id].displayName.GetLocalizedString();
                    //Setup Button
                    Button btn = go.GetComponentInChildren<Button>();
        ...
        public void PlayerModelSelectTeam(int team)
        {
            ...
            for (int i = 0; i < game.allPvpTeams[currentSelectedTeamForPlayerModel].playerModels.Length; i++)
            {
                ...
                //Setup text
                TextMeshProUGUI txt = go.GetComponentInChildren<TextMeshProUGUI>();
                if (txt) txt.text = game.allPvpTeams[currentSelectedTeamForPlayerModel].playerModels[id].displayName.GetLocalizedString();
                //Setup Button
                Button btn = go.GetComponentInChildren<Button>();
        ...
        public void PlayerModelSelect(int team, int id)
        {
            ...
            if (allLoadouts[currentlySelectedLoadout].teamLoadout[team].playerModelCustomizations.Length > 0)
            {
                ...
                for (int i = 0; i < model.customizationSlots.Length; i++)
                {
                    ...
                    //Setup text
                    TextMeshProUGUI txt = go.GetComponentInChildren<TextMeshProUGUI>();
                    if (txt) txt.text = model.customizationSlots[cId].name.GetLocalizedString();
                    //Setup Button
                    Button btn = go.GetComponentInChildren<Button>();
        ...
        public void PlayerModelCustomizeSlot(int team, int id, int slot)
        {
            ...
            for (int i = 0; i < model.customizationSlots[slot].customizations.Length; i++)
            {
                ...
                //Setup text
                TextMeshProUGUI txt = go.GetComponentInChildren<TextMeshProUGUI>();
                if (txt) txt.text = model.customizationSlots[slot].customizations[cId].name.GetLocalizedString();
                //Setup Button
                Button btn = go.GetComponentInChildren<Button>();
                    ...
                    if (game.allWeaponCategories.Length == allLoadouts[id].loadoutWeapons.Length)
                    {
                        for (int j = 0; j < allLoadouts[id].loadoutWeapons.Length; j++)
                        {
                            ...
                            if (savedWeapon < game.allWeapons.Length)
                            {
                                ...
                                if (game.allWeapons[savedWeapon].name == savedweaponName)
                                {
                    ...
                    if (amountOfTeams == allLoadouts[id].teamLoadout.Length)
                    {
                        for (int j = 0; j < allLoadouts[id].teamLoadout.Length; j++)
                        {
                            ...
                            if (savedPlayerModel < game.allPvpTeams[jd].playerModels.Length)
                            {
                                ...
                                if (game.allPvpTeams[jd].playerModels[savedPlayerModel].name == savedPlayerModelName)
                                {
                                    //It is! Assign it!
        ...
        public void Save()
        {
            if (wasLoaded)
            {
                for (int i = 0; i < allLoadouts.Length; i++)
                {
                    ...
                    {
                        int jd = j;
                        PlayerPrefs.SetString("loadout_" + id + "_weapon_" + jd + "_name", game.allWeapons[allLoadouts[id].loadoutWeapons[jd].weaponID].name);
                    ...
                    {
                        int jd = j;
                        PlayerPrefs.SetString("loadout_" + id + "_pm_" + jd + "_name", game.allPvpTeams[jd].playerModels[allLoadouts[id].teamLoadout[jd].playerModelID].name);
