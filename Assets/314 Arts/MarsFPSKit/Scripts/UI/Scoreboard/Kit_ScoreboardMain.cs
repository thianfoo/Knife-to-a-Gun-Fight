        ...
        void Redraw()
        {
            ...
            for (int i = 0; i < networkPlayerManager.players.Count; i++)
            {
                ...
                if (rt_ScoreboardEntries.Count > i)
                {
                    //Update
                    //Set name
                    rt_ScoreboardEntries[i].name = networkPlayerManager.players[i].name;

                    rt_ScoreboardEntries[i].team = networkPlayerManager.players[i].team;
                    rt_ScoreboardEntries[i].kills = networkPlayerManager.players[i].kills;
                    rt_ScoreboardEntries[i].assists = networkPlayerManager.players[i].assists;
                    rt_ScoreboardEntries[i].deaths = networkPlayerManager.players[i].deaths;
                    rt_ScoreboardEntries[i].ping = networkPlayerManager.players[i].ping;
                ...
                else
                {
                    ...
                    //Set name
                    entry.name = networkPlayerManager.players[i].name;
                    entry.team = networkPlayerManager.players[i].team;
                    entry.kills = networkPlayerManager.players[i].kills;
                    entry.assists = networkPlayerManager.players[i].assists;
                    entry.deaths = networkPlayerManager.players[i].deaths;
                    entry.ping = networkPlayerManager.players[i].ping;
            ...
            if (main.currentBotManager)
            {
                ...
                {
                    //Check if entry is available
                    if (rt_ScoreboardEntries.Count > i + networkPlayerManager.players.Count)
                    {
                        //Update
                        //Set name
                        rt_ScoreboardEntries[i + networkPlayerManager.players.Count].name = main.currentBotManager.bots[i].name;
                        //Copy team
                        rt_ScoreboardEntries[i + networkPlayerManager.players.Count].team = main.currentBotManager.bots[i].team;
                        //Copy kills
                        rt_ScoreboardEntries[i + networkPlayerManager.players.Count].kills = main.currentBotManager.bots[i].kills;
                        //Copy assists
                        rt_ScoreboardEntries[i + networkPlayerManager.players.Count].assists = main.currentBotManager.bots[i].assists;
                        //Copy Deaths
                        rt_ScoreboardEntries[i + networkPlayerManager.players.Count].deaths = main.currentBotManager.bots[i].deaths;
                        //Set ping to 0
                        rt_ScoreboardEntries[i + networkPlayerManager.players.Count].ping = 0;
                        //Set to used
                        rt_ScoreboardEntries[i + networkPlayerManager.players.Count].used = true;
                    }
                    else
                    {
                        //Create new
