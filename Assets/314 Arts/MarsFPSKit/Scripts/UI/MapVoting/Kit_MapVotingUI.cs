    ...
    public class Kit_MapVotingUI : Kit_MapVotingUIBase
    {
        ...
        public override void SetupVotes(Kit_MapVotingBehaviour behaviour)
        {
            ...
            for (int i = 0; i < behaviour.combos.Count; i++)
            {
                ...
                //Setup the new entry
                newEntry.gameModeName.text = main.gameInformation.allPvpGameModes[behaviour.combos[i].gameMode].gameModeName.GetLocalizedString();
                if (Kit_GameSettings.currentNetworkingMode == KitNetworkingMode.Traditional)
                {
                    newEntry.mapName.text = main.gameInformation.allPvpGameModes[behaviour.combos[i].gameMode].traditionalMaps[behaviour.combos[i].map].mapName;
                    newEntry.mapImage.sprite = main.gameInformation.allPvpGameModes[behaviour.combos[i].gameMode].traditionalMaps[behaviour.combos[i].map].mapPicture;
                }
                else if (Kit_GameSettings.currentNetworkingMode == KitNetworkingMode.Lobby)
                {
                    newEntry.mapName.text = main.gameInformation.allPvpGameModes[behaviour.combos[i].gameMode].lobbyMaps[behaviour.combos[i].map].mapName;
                    newEntry.mapImage.sprite = main.gameInformation.allPvpGameModes[behaviour.combos[i].gameMode].lobbyMaps[behaviour.combos[i].map].mapPicture;
                }
