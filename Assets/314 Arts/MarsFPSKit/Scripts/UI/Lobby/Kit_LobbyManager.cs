...
namespace MarsFPSKit.UI
{
    ...
    /// </summary>
    public class Kit_LobbyManager : Kit_Base
    {
        /// <summary>
        /// Main Menu
        /// </summary>
            ...
            if (PhotonNetwork.InRoom)
            {
                ...
                { 
                    ...
                    if (PhotonNetwork.IsMasterClient)
                    {
                        ...
                        while (mapsToVoteFor.Count < Mathf.Clamp(game.allPvpGameModes[gm].lobbyAmountOfMapsToVoteFor, 0, game.allPvpGameModes[gm].lobbyMaps.Length) && iterations < 100)
                        {
                            iterations++;
                            int mapToAdd = Random.Range(0, game.allPvpGameModes[gm].lobbyMaps.Length);
            ...
            if (Kit_GameSettings.currentNetworkingMode == KitNetworkingMode.Lobby)
            {
                if (PhotonNetwork.InRoom)
                {
                    ...
                    else if (Time.time > currentLobbyStateLastUpdate)
                    {
                        ...
                        {
                            if (PhotonNetwork.CurrentRoom.PlayerCount >= game.allPvpGameModes[(int)PhotonNetwork.CurrentRoom.CustomProperties["gameMode"]].lobbyMinimumPlayersNeeded)
                            {
                            ...
                            {
                                if (game.allPvpGameModes[(int)PhotonNetwork.CurrentRoom.CustomProperties["gameMode"]].lobbyStartWithBotsAfterSeconds > 0)
                                {
                                    if (PhotonNetwork.Time > (lobbyCreatedAtNetworkTime + (double)game.allPvpGameModes[(int)PhotonNetwork.CurrentRoom.CustomProperties["gameMode"]].lobbyStartWithBotsAfterSeconds))
                                    {
                                        if (!countdownUntilGameIsEnteredHasBegun)
                                        {
                                    ...
                                    else
                                    {
                                        ...
                                        if (rdn < 2)
                                        {
                                            if (((game.allPvpGameModes[(int)PhotonNetwork.CurrentRoom.CustomProperties["gameMode"]].lobbyMinimumPlayersNeeded) - PhotonNetwork.CurrentRoom.PlayerCount) > 1)
                                            {
                                                currentLobbyState.text = "We need " + ((game.allPvpGameModes[(int)PhotonNetwork.CurrentRoom.CustomProperties["gameMode"]].lobbyMinimumPlayersNeeded) - PhotonNetwork.CurrentRoom.PlayerCount).ToString() + " more players...";
                                            }
                                            else
                                            {
                            ...
                            if (countdownUntilGameIsEnteredTimeLeft > 0)
                            {
                                ...
                                if (countdownUntilGameIsEnteredTimeLeft <= 0)
                                {
                                    ...
                                    else
                                    {
                                        ...
                                        Hashtable roomTable = PhotonNetwork.CurrentRoom.CustomProperties;
                                        int gm = (int)PhotonNetwork.CurrentRoom.CustomProperties["gameMode"];
                                        roomTable["bots"] = game.allPvpGameModes[gm].lobbyBotsEnabled;
                                        PhotonNetwork.CurrentRoom.SetCustomProperties(roomTable);
                                    }
                                    ...
                                    if (mapVotes.Length > 0)
                                    {
                                        ...
                                        //Load the map
                                        sceneSyncer.LoadScene(game.allPvpGameModes[gameMode].lobbyMaps[mapToLoad].sceneName);
                                    }
                                    else
                                    {
                                        //Load Map
            ...
            if (PhotonNetwork.InRoom)
            {
                if (mapVotes.Length > 0)
                {
                    for (int i = 0; i < mapVotes.Length; i++)
                    {
                        ...
                        btn.img.sprite = game.allPvpGameModes[(int)PhotonNetwork.CurrentRoom.CustomProperties["gameMode"]].lobbyMaps[id].mapPicture;
                ...
                else
                {
                    ...
                    Kit_LobbyButton btn = go.GetComponent<Kit_LobbyButton>();
                    btn.txt.text = "";
                    btn.img.sprite = game.allPvpGameModes[(int)PhotonNetwork.CurrentRoom.CustomProperties["gameMode"]].lobbyMaps[(int)PhotonNetwork.CurrentRoom.CustomProperties["map"]].mapPicture;
                    mapVoteEntries.Add(btn);
                }
            }
        }
        ...
        public void RedrawGameModeSelection()
        {
            ...
            int[] playersInGameModeRoom = new int[game.allPvpGameModes.Length];
            ...
            for (int i = 0; i < game.allPvpGameModes.Length; i++)
            {
                gameModeSelectionEntries[i].txt.text = game.allPvpGameModes[i].gameModeName + " [" + playersInGameModeRoom[i] + "/" + totalPlayersInAllRooms + "]";
            }
        }
        ...
        public void SearchGame(int gameMode)
        {
            if (PhotonNetwork.IsConnectedAndReady && PhotonNetwork.InLobby)
            {
                ...
                //Assign settings
                //Player Limit
                options.MaxPlayers = game.allPvpGameModes[gameMode].lobbyMaximumPlayers;
                //Create a new hashtable
                options.CustomRoomProperties = new Hashtable();
                //Lobby or not
                options.CustomRoomProperties.Add("lobby", true);
                //Map
                options.CustomRoomProperties.Add("map", Random.Range(0, game.allPvpGameModes[gameMode].lobbyMaps.Length));
                //Game Mode
                ...
                //Bots
                options.CustomRoomProperties.Add("bots", game.allPvpGameModes[gameMode].lobbyBotsEnabled);
                //Password
                options.CustomRoomProperties.Add("password", "");
                //Player needed
                options.CustomRoomProperties.Add("playerNeeded", game.allPvpGameModes[gameMode].lobbyMinimumPlayersNeeded);
