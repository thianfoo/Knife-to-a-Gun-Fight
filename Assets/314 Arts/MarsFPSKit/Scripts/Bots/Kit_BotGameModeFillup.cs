...
namespace MarsFPSKit
{
    ...
    public class Kit_BotGameModeFillup : Kit_BotGameModeManagerBase
    {
        public int MaxPlayers
        {
            get
            {
                return networkManager.maxConnections;
            }
        }
        ...
        public override void Inizialize(Kit_BotManager manager)
        {
            if (main.currentPvPGameModeBehaviour.isTeamGameMode)
            {
                ...
                for (sbyte i = 0; i < Mathf.Clamp(main.gameInformation.allPvpTeams.Length, 0, main.currentPvPGameModeBehaviour.maximumAmountOfTeams); i++)
                {
                    ...
                    //Reset tries
                    tries = 0;
                    if (manager.GetPlayersInTeamX(i) + manager.GetBotsInTeamX(i) < MaxPlayers / Mathf.Clamp(main.gameInformation.allPvpTeams.Length, 0, main.currentPvPGameModeBehaviour.maximumAmountOfTeams))
                    {
                    ...
                    else if (manager.GetPlayersInTeamX(i) + manager.GetBotsInTeamX(i) > MaxPlayers / Mathf.Clamp(main.gameInformation.allPvpTeams.Length, 0, main.currentPvPGameModeBehaviour.maximumAmountOfTeams))
                    {
                        while (manager.GetPlayersInTeamX(i) + manager.GetBotsInTeamX(i) > MaxPlayers / Mathf.Clamp(main.gameInformation.allPvpTeams.Length, 0, main.currentPvPGameModeBehaviour.maximumAmountOfTeams) && tries <= 20)
                        {
