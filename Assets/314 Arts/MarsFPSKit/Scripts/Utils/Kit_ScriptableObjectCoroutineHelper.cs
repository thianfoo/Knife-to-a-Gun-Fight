...
namespace MarsFPSKit
{
    ...
    public class Kit_ScriptableObjectCoroutineHelper : Kit_Base
    {
        ...
        public IEnumerator NetworkReplaceWeapon(Kit_PlayerBehaviour pb, Kit_ModernWeaponManagerNetworkData runtimeData, int itemIndex, uint newItem)
        {
            ...
            {
                while (!NetworkServer.spawned.ContainsKey(newItem)) yield return null;
            ...
            Kit_WeaponRuntimeDataBase wpnNetData = id.GetComponent<Kit_WeaponRuntimeDataBase>();
            wpnNetData.pb = pb;
            runtimeData.weaponsInUseDataObjects[itemIndex] = wpnNetData;

            int weaponId = wpnNetData.id;
            //Get their behaviour modules
            Kit_WeaponBase weaponBehaviour = pb.gameInformation.allWeapons[weaponId];
            //Assign Behaviour
            wpnNetData.behaviour = weaponBehaviour;
            //Setup FP
            weaponBehaviour.SetupFirstPerson(pb, wpnNetData);
            //Setup TP
            weaponBehaviour.SetupThirdPerson(pb, wpnNetData);
            ...
            if (!runtimeData.quickUseInProgress && itemIndex == runtimeData.currentWeapon)
            {
                ...
                wpnNetData.behaviour.DrawWeapon(pb, wpnNetData);
                //Play Third person animation
                pb.thirdPersonPlayerModel.PlayWeaponChangeAnimation(wpnNetData.behaviour.thirdPersonAnimType, true, wpnNetData.behaviour.drawTime);
                runtimeData.switchInProgress = true;
                //Set phase
                runtimeData.switchPhase = 1;
                //Set time
                runtimeData.switchNextEnd = Time.time + wpnNetData.behaviour.drawTime;
                //Play Third person animation
                pb.thirdPersonPlayerModel.PlayWeaponChangeAnimation(wpnNetData.behaviour.thirdPersonAnimType, true, wpnNetData.behaviour.drawTime);
            }
        }
        ...
        public IEnumerator MeleeExecuteStab(Kit_ModernMeleeScript values, Kit_ModernMeleeScriptRuntimeData data, AttackSettings attackSettings, Kit_PlayerBehaviour pb, float revertBy)
        {
            ...
            if (pb)
            {
                ...
                RaycastHit[] hits = Physics.BoxCastAll(center, attackSettings.stabHalfExtents, dir, Quaternion.LookRotation(dir), attackSettings.stabReach, pb.gameInformation.weaponHitLayers.value, QueryTriggerInteraction.Collide).OrderBy(h => Vector3.Distance(pb.playerCameraTransform.position, h.point)).ToArray();
                ...
                for (int i = 0; i < hits.Length; i++)
                {
                    ...
                    if (hits[i].transform.root != pb.transform.root)
                    {
                        ...
                        if (hits[i].transform.GetComponent<Kit_PlayerDamageMultiplier>() && main.currentGameModeType == 2)
                        {
                            ...
                            if (hits[i].transform.root.GetComponent<Kit_PlayerBehaviour>())
                            {
                                ...
                                if (main.currentPvPGameModeBehaviour.ArePlayersEnemies(pb, hitPb))
                                {
        ...
        public IEnumerator MeleeExecuteCharge(Kit_ModernMeleeScript values, Kit_ModernMeleeScriptRuntimeData data, AttackSettings attackSettings, Kit_PlayerBehaviour pb, float revertBy)
        {
            ...
            if (pb)
            {
                ...
                RaycastHit[] hits = Physics.BoxCastAll(center, attackSettings.chargeHalfExtents, dir, Quaternion.LookRotation(dir), attackSettings.chargeReach, pb.gameInformation.weaponHitLayers.value, QueryTriggerInteraction.Collide).OrderBy(h => Vector3.Distance(pb.playerCameraTransform.position, h.point)).ToArray();
