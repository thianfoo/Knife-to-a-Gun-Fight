...
{
    public class Kit_FlashbangExplosion : Kit_Base
    {
        /// <summary>
        /// Explosion radius
        /// </summary>
                ...
                if (NetworkServer.active)
                {
                    if (affectedByExplosion[i].GetComponent<Kit_PlayerDamageMultiplier>())
                    {
                        ...
                        if (affectedByExplosion[i].transform.root.GetComponent<Kit_PlayerBehaviour>())
                        {
                            ...
                            {
                                if (!main.currentPvPGameModeBehaviour || (main.currentPvPGameModeBehaviour && main.currentPvPGameModeBehaviour.ArePlayersEnemies(idWhoShot, botShot, player, true)))
                                {
                                    if (!blindedPlayers.Contains(player))
                                    {
                                        ...
                                        if (!botShot)
                                        {
                                            Kit_Player nPlayer = networkPlayerManager.GetPlayerById(idWhoShot);

                                            if (nPlayer != null)
                                            {
                                                main.TargetHitmarker(nPlayer.serverToClientConnection, 0);
                                            }
                                        }
                                    }
                                }
