...
{
    public class Kit_Explosion : Kit_Base
    {
        /// <summary>
        /// Explosion radius
        /// </summary>
        ...
        void Start()
        {
            ...
            {
                float dist = Vector3.Distance(main.cameraManager.mainCamera.transform.position, transform.position);
                if (dist < shakeDistance)
                {
                    main.cameraManager.cameraShake.ShakeCamera(Mathf.Lerp(shakeAmount, 0f, dist / shakeDistance), Mathf.Lerp(shakeDuration, 0f, dist / shakeDistance));
                }
            }
                ...
                if (NetworkServer.active)
                {
                    if (affectedByExplosion[i].GetComponent<Kit_PlayerDamageMultiplier>())
                    {
                        ...
                        if (affectedByExplosion[i].transform.root.GetComponent<Kit_PlayerBehaviour>())
                        {
                            ...
                            {
                                if ((!main.currentPvPGameModeBehaviour && player.id == idWhoShot && player.isBot == botShot) || (main.currentPvPGameModeBehaviour && main.currentPvPGameModeBehaviour.ArePlayersEnemies(idWhoShot, botShot, player, true)))
                                {
                                    ...
                                    if (!botShot)
                                    {
                                        Kit_Player targetForHitmarker = networkPlayerManager.GetPlayerById(idWhoShot);
                                        main.TargetHitmarker(targetForHitmarker.serverToClientConnection, 0);
                                    }
                                }
                            }
                        }
