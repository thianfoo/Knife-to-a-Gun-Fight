...
namespace MarsFPSKit
{
    ...
    public class Kit_PvP_GMB_GunGame : Kit_PvP_GameModeBase
    {
        ...
        {
            Kit_PvP_GMB_GunGameNetworkData drd = main.currentGameModeRuntimeData as Kit_PvP_GMB_GunGameNetworkData;
        ...
        void CheckForWinner()
        {
            ...
            if (main.gameModeStage < 2)
            {
                ...
                tempPlayers.AddRange(networkPlayerManager.players);
        ...
        {
            DebugLog("Game Mode received kill");
            ...
            else
            {
                ...
                {
                    Kit_PlayerBehaviour playerInstance = networkPlayerManager.GetPlayerBehaviourById(killer);
                    //Set weapon on our player
                    if (playerInstance)
                    {
                        Kit_WeaponBase weaponInfo = main.gameInformation.allWeapons[weaponOrders[drd.currentGunOrder].weapons[drd.currentGun[killer]]];
                        if (weaponInfo.GetType() == typeof(Kit_ModernWeaponScript))
                        {
                            ...
                            playerInstance.ServerReplaceWeapon(0, weaponOrders[drd.currentGunOrder].weapons[drd.currentGun[killer]], weapon.bulletsPerMag, weapon.bulletsToReloadAtStart, attachments);
                        }
                        else
                        {
                            playerInstance.ServerReplaceWeapon(0, weaponOrders[drd.currentGunOrder].weapons[drd.currentGun[killer]], 0, 0, new int[0]);
                        }
                    }
                }
            }
        ...
        public override void TimeRunOut()
        {
            ...
            else if (main.gameModeStage == 3)
            {
                ...
                //Delete map voting
                NetworkServer.Destroy(main.currentMapVoting.gameObject);
        ...
        public override bool CanSpawn(Kit_Bot bot)
        {
            ...
            {
                //Check if it is a valid team
                if (bot.team >= 0 && bot.team < main.gameInformation.allPvpTeams.Length) return true;
            }
            return false;
        }
        ...
        public override GameObject DoCustomSpawn(Kit_Player player, Loadout selectedLoadout)
        {
            if (CanSpawn(player))
            {
                ...
                if (spawnLocation)
                {
                    ...
                    Loadout loadout = main.loadoutMenu.GetCurrentLoadout();
                    ...
                    DebugLog("Current gun:" + currentGun);
                    if (weaponOrders[drd.currentGunOrder].weapons.Length > currentGun)
                    {
                        ...
                        if (main.gameInformation.allWeapons[loadout.loadoutWeapons[0].weaponID].GetType() == typeof(Weapons.Kit_ModernWeaponScript))
                        {
                            Weapons.Kit_ModernWeaponScript weapon = main.gameInformation.allWeapons[weaponOrders[drd.currentGunOrder].weapons[currentGun]] as Weapons.Kit_ModernWeaponScript;
                            loadout.loadoutWeapons[0].attachments = new int[weapon.attachmentSlots.Length];
                        }

                        GameObject go = Instantiate(main.playerPrefab, spawnLocation.position, spawnLocation.rotation);
