...
namespace MarsFPSKit
{
    namespace UI
    {
        public class Kit_PvE_SandboxMenu : Kit_MenuPveGameModeBase
        {
            ...
            /// <summary>
            /// Prefab for network data
            /// </summary>
            public GameObject coopMenuNetworkDataPrefab;

            /// <summary>
            /// Coop network data
            /// </summary>
            public Kit_PvE_SandboxMenuNetworkData coopNetworkData;
            ...
            {
                networkManager.onHostStartedEvent.AddListener(OnCreatedRoom);
            }
            ...
            private void OnDisable()
            {
                if (networkManager)
                {
                    networkManager.onHostStartedEvent.RemoveListener(OnCreatedRoom);
                }
            }
            ...
            public void SingleplayerNextMap()
            {
                ...
                if (spCurMap >= game.allSingleplayerGameModes[myId].maps.Length)
                {
                    spCurMap = 0;
                }
            ...
            public void SingleplayerPreviousMap()
            {
                ...
                if (spCurMap < 0)
                {
                    spCurMap = game.allSingleplayerGameModes[myId].maps.Length - 1;
                }
            ...
            public void CoopHostGame()
            {
                ...
                networkManager.onHostStartedEvent.AddListener(delegate
                {
                    ...
                    gameInfo.playerLimit = game.allCoopGameModes[myId].coopPlayerAmount;
                    networkManager.maxConnections = game.allCoopGameModes[myId].coopPlayerAmount;
                    gameInfo.map = 0;
                    ...
                    GameObject menuData = Instantiate(coopMenuNetworkDataPrefab);
                    NetworkServer.Spawn(menuData);
                });
                networkManager.onHostStartedEvent.AddListener(OnCreatedRoom);

                //Start
                networkManager.HostGame(game.allCoopGameModes[myId].coopPlayerAmount);
            }
            ...
            public void CoopStart()
            {
                ...
                {
                    //Load the map
                    sceneSyncer.LoadScene(game.allCoopGameModes[myId].maps[coopNetworkData.coopCurMap].sceneName);
                }
            }
            ...
            public void CoopNextMap()
            {
                if (NetworkServer.active)
                {
                    int map = coopNetworkData.coopCurMap;

                    map++;

                    if (map >= game.allCoopGameModes[myId].maps.Length)
                    {
                        map = 0;
                    }

                    coopNetworkData.coopCurMap = map;
            ...
            public void CoopPreviousMap()
            {
                if (NetworkServer.active)
                {
                    ...
                    if (map < 0)
                    {
                        map = game.allCoopGameModes[myId].maps.Length - 1;
                    }
            ...
            public override void RegisterCustomPrefabs()
            {
                if (!NetworkClient.prefabs.ContainsKey(coopMenuNetworkDataPrefab.GetComponent<NetworkIdentity>().assetId))
                {
                    NetworkClient.RegisterPrefab(coopMenuNetworkDataPrefab);
                }
            }
            ...
            {
                spMapName.text = game.allSingleplayerGameModes[myId].maps[spCurMap].mapName;
            }
            ...
            private void RedrawCoopMenu()
            {
                ...
                if (!coopNetworkData) return;

                coopMapName.text = game.allCoopGameModes[myId].maps[coopNetworkData.coopCurMap].mapName;
                ...
                if (NetworkClient.active)
                {
                    ...
                    for (int i = 0; i < networkPlayerManager.players.Count; i++)
                    {
                        ...
                        txt.text = networkPlayerManager.players[i].name;
            ...
            {
                game.masterServer.RefreshGames();
                var games = game.masterServer.GetGames();
                ...
                foreach (var info in games)
                {
                    ...
                    if (info.gameModeType == 1)
                    {
                        ...
                        if (info.gameMode == myId)
                        {
                            ...
                            entry.Setup(this, info, delegate { JoinRoom(info); });
                            //Add it to our active list so it will get cleaned up next time
            ...
            public void OnCreatedRoom()
            {
                ...
                if (networkGameInformation.gameModeType == 0) //Singleplayer - load map - coop does it on button click
                {
                    if (networkGameInformation.gameMode == myId)
                    {
                        //Load the map
                        sceneSyncer.LoadScene(game.allSingleplayerGameModes[myId].maps[networkGameInformation.map].sceneName);
                    }
                }

                if (networkGameInformation.gameModeType == 1) //COOP
                {
                    if (networkGameInformation.gameMode == myId)
                    {
