...
namespace MarsFPSKit
{
    ...
    public class Kit_IngameMain : Kit_BaseNetworked
    {
        public static Kit_IngameMain instance { get; private set; }
        ...
        [Header("Modules")]
        [Header("Camera")]
        public Kit_CameraManager cameraManager;
        ...
        #region Scoreboard
        [Header("Scoreboard")]
        public Kit_ScoreboardBase scoreboard;
        #endregion
        ...
        public static bool ExecuteDestroyActions
        {
            get
            {
                return instance && instance.executeDestroyActions;
            }
        }

        public bool executeDestroyActions
        {
            get
            {
                return !instance.isShuttingDown && sceneSyncer && !sceneSyncer.isLoading;
            }
        }
        #endregion
        ...
        void Awake()
        {
            ...
            for (int i = 0; i < menuScreens.Length; i++)
            {
                ...
                {
                    DebugLogError("Menu root at index " + i + " is not assigned.", this);
                }
            }

            //Make sure the event system is enabled
        ...
        void Update()
        {
            ...
            if (NetworkServer.active || NetworkClient.active)
            {
                ...
                {
                    Time.timeScale = 1f;
                    DebugLogWarning("Timescale must remain 1");
                }
        ...
        public override void OnStartServer()
        {
            ...
            {
                //Get type
                int gameModeType = networkGameInformation.gameModeType;
                //Assign
                ...
                if (gameModeType == 0)
                {
                    ...
                    int gameMode = networkGameInformation.gameMode;
                ...
                else if (gameModeType == 2)
                {
                    ...
                    //Set timer
                    int duration = networkGameInformation.duration;
                    ...
                    {
                        //Get ping limit
                        int pingLimit = networkGameInformation.ping;
                        ...
                        //Get AFK limit
                        int afkLimit = networkGameInformation.afk;
                    ...
                    if (networkGameInformation.bots)
                    {
                        ...
                        else
                        {
                            if (!currentBotManager)
                            {
                                currentBotManager = FindAnyObjectByType<Kit_BotManager>();
                            }
                        }
                    }
            ...
            {
                DebugLogError("No Game Information assigned. Game will not work.");
            }
        }
        ...
        public override void OnStopClient()
        {
            ...
            {
                sceneSyncer.LoadScene("MainMenu", true);
            }
        }
        #endregion
        ...
        {
            DebugLog("Player: " + player.name + " left the server");
        ...
        {
            DebugLog("Player: " + newPlayer.name + " joined the server");
        ...
        public void RpcDeathInfo(bool botShot, uint killer, bool botKilled, uint killed, int gunId, int thirdPersonPlayerModelId, int ragdollId)
        {
            if (isClient)
            {
                ...
                {
                    if (killed == networkPlayerManager.myId)
                    {
                        for (int i = 0; i < gameInformation.plugins.Length; i++)
                        {
        ...
        public void RpcDeathInfo(bool botShot, uint killer, bool botKilled, uint killed, string cause, int thirdPersonPlayerModelId, int ragdollId)
        {
            if (isClient)
            {
                ...
                {
                    if (killer == networkPlayerManager.myId && (botKilled || killed != networkPlayerManager.myId))
                    {
                        //Display points
        ...
        {
            Kit_Player sender = networkPlayerManager.GetPlayerByConnection(who);
            if (sender != null)
            {
                //Relay to all clients
                RpcChatMessage(false, sender.id, msg, type);
                //Debug the message
                DebugLog("Chat message from: " + sender.name + ": " + msg);
            }
        }
        ...
        public void RpcChatMessage(bool bot, uint who, string msg, byte type)
        {
            ...
            {
                Kit_Player sender = networkPlayerManager.GetPlayerById(who);
                if (sender != null)
                {
                    chat.DisplayChatMessage(sender, msg, type);
                }
            ...
            if (isServer)
            {
                ...
                if (botKilled)
                {
                    if (isServer && currentBotManager)
                    {
                        ...
                        main.currentBotManager.ModifyBotData(killedBot);
                    }
                }
                else
                {
                    Kit_Player player = networkPlayerManager.GetPlayerById(killed);
                    player.deaths++;
                ...
                if (botShot)
                {
                    ...
                    if (!botKilled || botKilled && killer != killed)
                    {
                        if (isServer && currentBotManager)
                        {
                            ...
                            main.currentBotManager.ModifyBotData(killerBot);
                        }
                    }
                }
                else
                {
                    if (botShot != botKilled || killed != killer)
                    {
                        Kit_Player player = networkPlayerManager.GetPlayerById(killer);
                        player.kills++;
                        networkPlayerManager.ModifyPlayerData(player);
                    }
                }
                ...
                if (isServer)
                {
                    ...
                    for (int i = 0; i < gameInformation.plugins.Length; i++)
                    {
                        var plugin = gameInformation.plugins[i];
                        plugin.PlayerScoredKill(botShot, killer, botKilled, killed, gunId, thirdPersonPlayerModelId, ragdollId);
                    }
                }
            }
        }
        ...
        public void ServerDeathInfo(bool botShot, uint killer, bool botKilled, uint killed, string cause, int thirdPersonPlayerModelId, int ragdollId)
        {
            if (isServer)
            {
                ...
                {
                    var plugin = gameInformation.plugins[i];
                    plugin.PlayerScoredKill(botShot, killer, botKilled, killed, cause, thirdPersonPlayerModelId, ragdollId);
                }
            }
        }
        ...
        {
            Kit_Player player = networkPlayerManager.GetPlayerByConnection(sender);
                        ...
                        if (!pb)
                        {
                            ...
                            if (!currentPvEGameModeBehaviour.UsesCustomSpawn())
                            {
                                if (gameInformation.allSingleplayerGameModes[currentGameMode].CanSpawn(player))
                                {
                                    ...
                                    Loadout loadout = GetLoadoutWithInjectedWeapons(gameInformation.allSingleplayerGameModes[currentGameMode].GetSpawnLoadout(player));
                                    if (spawnLocation)
                                    {
                        ...
                        if (!pb)
                        {
                            ...
                            if (!currentPvEGameModeBehaviour.UsesCustomSpawn())
                            {
                                if (gameInformation.allCoopGameModes[currentGameMode].CanSpawn(player))
                                {
                                    ...
                                    Loadout loadout = GetLoadoutWithInjectedWeapons(gameInformation.allCoopGameModes[currentGameMode].GetSpawnLoadout(player));
                                    if (spawnLocation)
                                    {
                                        GameObject go = Instantiate(playerPrefab, spawnLocation.position, spawnLocation.rotation);
                                        pb = go.GetComponent<Kit_PlayerBehaviour>();
                    ...
                    case 2:
                        ...
                        if (!pb && player.team >= 0)
                        {
                            ...
                            if (!currentPvPGameModeBehaviour.UsesCustomSpawn())
                            {
                                if (gameInformation.allPvpGameModes[currentGameMode].CanSpawn(player))
                                {
                                    ...
                                    if (spawnLocation)
                                    {
                                        ...
                                        //IMPROVE:
                                        Loadout loadout = GetLoadoutWithInjectedWeapons(requestedLoadout);
                                        GameObject go = Instantiate(playerPrefab, spawnLocation.position, spawnLocation.rotation);
                                        pb = go.GetComponent<Kit_PlayerBehaviour>();
                                        pb.myTeam = player.team;
                                        pb.isBot = false;
                            ...
                            else
                            {
                                ...
                                GameObject playerGameObject = currentPvPGameModeBehaviour.DoCustomSpawn(player, GetLoadoutWithInjectedWeapons(requestedLoadout));
                                if (playerGameObject)
                                {
        ...
        private Loadout GetLoadoutWithInjectedWeapons(Loadout baseLoadout)
        {
            var playerScriptFromPrefab = playerPrefab.GetComponent<Kit_PlayerBehaviour>();
            var weaponList = baseLoadout.loadoutWeapons.ToList();
            ...
            for (int i = 0; i < gameInformation.plugins.Length; i++)
            {
                WeaponsFromPlugin weapons = gameInformation.plugins[i].WeaponsToInjectIntoWeaponManager();
                if (weapons.weaponsInSlot.Length > 0)
                {
                    int id = i;
                    pluginWeaponsCallback.Add(gameInformation.plugins[id]);
                    weaponsFromPlugins.Add(weapons);
                }
            }

            WeaponsFromPlugin movementWeapons = playerScriptFromPrefab.movement.WeaponsToInjectIntoWeaponManager();
            if (movementWeapons != null && movementWeapons.weaponsInSlot.Length > 0)
            {
                pluginWeaponsCallback.Add(playerScriptFromPrefab.movement);
                weaponsFromPlugins.Add(movementWeapons);
            }

            if (currentPvPGameModeBehaviour)
            {
                WeaponsFromPlugin gameModeWeapons = currentPvPGameModeBehaviour.WeaponsToInjectIntoWeaponManager();
                if (gameModeWeapons != null && gameModeWeapons.weaponsInSlot.Length > 0)
                {
                    pluginWeaponsCallback.Add(currentPvPGameModeBehaviour);
            ...
            if (currentPvEGameModeBehaviour)
            {
                WeaponsFromPlugin gameModeWeapons = currentPvEGameModeBehaviour.WeaponsToInjectIntoWeaponManager();
                if (gameModeWeapons != null && gameModeWeapons.weaponsInSlot.Length > 0)
                {
                    pluginWeaponsCallback.Add(currentPvEGameModeBehaviour);
                    weaponsFromPlugins.Add(gameModeWeapons);
                }
            }
            ...
            {
                var wpns = weaponsFromPlugins[i];
                for (int j = 0; j < wpns.weaponsInSlot.Length; j++)
                {
                    LoadoutWeapon lw = new LoadoutWeapon();
                    lw.weaponID = System.Array.IndexOf(gameInformation.allWeapons, weaponsFromPlugins[i].weaponsInSlot[j].weapon);
                    if (lw.weaponID >= 0)
                    {
                        lw.attachments = weaponsFromPlugins[i].weaponsInSlot[j].attachments;
                        weaponList.Add(lw);
                    }
                    else
                    {
                        DebugLogError("Trying to inject a weapon that is not present in the weapon list of the game information!", weaponsFromPlugins[i].weaponsInSlot[j].weapon);
                    }
                }
            }
            ...
            newLoadout.teamLoadout = baseLoadout.teamLoadout;
            newLoadout.loadoutWeapons = weaponList.ToArray();
        ...
        public void TeamJoinGranted(NetworkConnection target, sbyte teamID)
        {
            ...
            {
                DebugLogError("Voice Chat Error: " + e);
            }
            //Minimap callback
            if (minimap)
            {
        ...
        [TargetRpc]
        public void TargetPoints(NetworkConnection target, int points, PointType pt)
        {
            if (pointsUI)
            {
                pointsUI.DisplayPoints(points, pt);
            }

            gameInformation.leveling.AddXp(points);
        }
        ...
        public void RpcAssistEvent(byte eventCode, int content)
        {
            ...
            if (assistManager)
            {
                assistManager.ClientOnGenericEvent(eventCode, content);
            }
        }
        ...
        [TargetRpc]
        public void TargetGenericEvent(NetworkConnectionToClient target, byte eventCode, object content)
        {
            for (int i = 0; i < gameInformation.plugins.Length; i++)
            {
                gameInformation.plugins[i].OnTargetGenericEvent(target, eventCode, content);
            }
        }
        ...
        public void SwitchMap(int to)
        {
            ...
            {
                //Update settings
                if (networkGameInformation)
                {
                    networkGameInformation.map = to;
                }

                if (Kit_GameSettings.currentNetworkingMode == KitNetworkingMode.Traditional)
                {
                    //Load the map
                    sceneSyncer.LoadScene(currentPvPGameModeBehaviour.traditionalMaps[to].sceneName);
                }
                else if (Kit_GameSettings.currentNetworkingMode == KitNetworkingMode.Lobby)
                {
                    //Load the map
                    sceneSyncer.LoadScene(currentPvPGameModeBehaviour.lobbyMaps[to].sceneName);
                }
            }
        }
        ...
        public void SwitchGameMode(int to)
        {
            if (isServer && currentGameModeType == 2)
            {
                ...
                if (networkGameInformation)
                {
                    networkGameInformation.gameMode = to;
                }
        ...
        public void ResumeButton()
        {
            ...
            else if (currentPvPGameModeBehaviour && currentPvPGameModeBehaviour.CanSpawn(networkPlayerManager.GetLocalPlayer()))
            {
                //We haven't, try to spawn
                Spawn();
            }
            else if (currentPvEGameModeBehaviour && currentPvEGameModeBehaviour.CanSpawn(networkPlayerManager.GetLocalPlayer()))
            {
                //We haven't, try to spawn
                Spawn();
            }
        ...
        public void ResetAllStatsEndOfRound()
        {
            if (isServer)
            {
                ...
                {
                    networkPlayerManager.players[i].team = -1;
                    networkPlayerManager.players[i].kills = 0;
                    networkPlayerManager.players[i].assists = 0;
                    networkPlayerManager.players[i].deaths = 0;
                }
        ...
        {
            DebugLog("Requested switch from " + currentScreen + " to " + newMenu);
        ...
        {
            DebugLog("Requested switch from " + currentScreen + " to " + newMenu + ". Force? " + force);
