...
{
    public class Kit_NetworkPlayerManager : Kit_BaseNetworked
    {
        public static Kit_NetworkPlayerManager instance { get; private set; }
        ...
        public UnityEvent<Kit_Player> onPlayerEvent = new UnityEvent<Kit_Player>();
            ...
            try
            {
                switch (op)
                {
                    case SyncList<Kit_Player>.Operation.OP_ADD:
                        ...
                        if (networkGameInformation && networkManager.game.masterServer && !networkManager.isOfflineMode)
                        {
                            networkManager.game.masterServer.GameServerInfoUpdated(networkGameInformation);
                        }
                        //Generic callback
                        ...
                        if (main)
                        {
                            ...
                            {
                                main.currentPvPGameModeBehaviour.OnPlayerJoined(item);
                            }

                            if (main.currentPvEGameModeBehaviour)
                            {
                                main.currentPvEGameModeBehaviour.OnPlayerJoined(item);
                            }
                        }
                    ...
                    case SyncList<Kit_Player>.Operation.OP_REMOVEAT:
                        ...
                        if (main)
                        {
                            if (main.currentPvPGameModeBehaviour)
                            {
                                main.currentPvPGameModeBehaviour.OnPlayerLeft(item);
                            }

                            if (main.currentPvEGameModeBehaviour)
                            {
                                main.currentPvEGameModeBehaviour.OnPlayerLeft(item);
                            }
                        }
                ...
                onPlayerEvent.Invoke(item);
            }
            catch (System.Exception e)
            {
                DebugLogWarning("OnPlayersChanged Exception: " + e.ToString());
            }
        }
        ...
        {
            return main.allActivePlayers.Where(x => !x.isBot && x.id == id).FirstOrDefault();
        }
        ...
        public Kit_Player GetLocalPlayer()
        {
            ...
            {
                if (!players[i].isBot && players[i].id == myId) return players[i];
            }
        ...
        public Kit_PlayerBehaviour GetPlayerObjectByConnection(NetworkConnectionToClient conn)
        {
            if (!main) return null;

            return main.allActivePlayers.Where(x => !x.isBot && x.connectionToClient == conn).FirstOrDefault();
        }
    }
}